# RESUMEN EJECUTIVO

## Objeto del informe

**El presente informe tiene por objeto la evaluación técnica de la propuesta presentada por la empresa empresa_s** en el marco del procedimiento basado en el Sistema Dinámico de Adquisición SDA 26/2021, relativo a los servicios de desarrollo evolutivo, mantenimiento correctivo, soporte técnico y mejora continua del ecosistema tecnológico EducaMadrid.

**La valoración se realiza conforme a los criterios establecidos en el Documento de Invitación, en particular los recogidos en su apartado relativo a los criterios sujetos a juicio de valor.** El análisis se ha efectuado exclusivamente sobre el contenido de la memoria técnica presentada por el licitador, sin considerar aspectos de naturaleza económica.

## Metodología de evaluación

La evaluación se ha estructurado en dos niveles complementarios.

**En primer lugar, se ha llevado a cabo un análisis técnico exhaustivo del contenido de la propuesta, abordando de manera individualizada los diferentes proyectos y sistemas incluidos en el Anexo II del pliego.** Este análisis ha permitido identificar el grado de desarrollo técnico de las soluciones planteadas, así como su adecuación al entorno real de EducaMadrid.

**En segundo lugar, los resultados de dicho análisis se han trasladado al esquema de valoración definido en el pliego**, asignando las puntuaciones correspondientes en cada uno de los subcriterios en función del nivel de adecuación observado.

Este enfoque garantiza la trazabilidad entre los requisitos definidos, las soluciones propuestas y la puntuación final asignada.

## Síntesis técnica de la propuesta

La propuesta presentada por empresa_s se caracteriza por un elevado grado de madurez técnica, una estructuración coherente de los contenidos y un nivel de detalle que permite su evaluación objetiva en todos los ámbitos analizados.

Desde el punto de vista técnico, la propuesta desarrolla de forma consistente los principales elementos requeridos en el pliego, destacando especialmente:

- Una **arquitectura claramente definida**, basada en principios de modularidad, desacoplamiento y control de operaciones, que permite su adaptación a entornos complejos y heterogéneos.

- Un **alto grado de comprensión del entorno EducaMadrid**, abordado desde una perspectiva integral que considera tanto la dimensión tecnológica como la operativa.

- La **definición de soluciones específicas** para los distintos sistemas incluidos en el Anexo II, evitando enfoques genéricos o excesivamente abstractos.

- La **incorporación de mecanismos de control, trazabilidad y validación** que permiten garantizar la correcta ejecución de las actuaciones propuestas.

- Una **planificación estructurada y coherente**, basada en fases, hitos y mecanismos de control temporal, que facilita la ejecución ordenada del contrato.

Asimismo, la propuesta **integra de manera adecuada los requisitos no funcionales**, incluyendo aspectos de rendimiento, seguridad, escalabilidad, mantenibilidad y disponibilidad, incorporándolos como elementos estructurales del diseño de la solución.

## Principales conclusiones del análisis

El análisis técnico realizado permite concluir que la propuesta de empresa_s **presenta un alto grado de adecuación a los requisitos definidos en el pliego**, tanto en términos de cobertura funcional como de desarrollo técnico.

A diferencia de otras propuestas analizadas en el mismo procedimiento, en este caso se observa:

- La existencia de **soluciones técnicas concretas y desarrolladas**.

- La **ausencia de errores conceptuales relevantes** en la definición de los sistemas.

- La adecuada **alineación con las arquitecturas tecnológicas empleadas** en el entorno real.

- La definición explícita de **mecanismos de integración, validación y control**.

Adicionalmente, la propuesta permite evaluar de manera objetiva su viabilidad, al incluir información suficiente para verificar la forma en que se ejecutarán las actuaciones previstas.

La combinación de estos factores permite considerar que la propuesta ofrece garantías suficientes para asegurar el cumplimiento del objeto del contrato en condiciones de calidad.

## Resultado de la valoración

En aplicación del baremo definido en el pliego, la propuesta ha sido valorada en los siguientes términos:

- **Solución técnica ofertada: 13,60 puntos sobre 15**

- **Planificación del servicio: 13,10 puntos sobre 15**

Como resultado global, la puntuación final asciende a:

- **26,70 puntos sobre un máximo de 30 puntos**

## Conclusión del resumen ejecutivo

La propuesta técnica presentada por empresa_s se sitúa en un **nivel alto de calidad dentro del conjunto de ofertas analizadas**, destacando por su coherencia interna, su desarrollo técnico y su capacidad de adaptación al entorno real del contrato.

El contenido de la memoria permite verificar de manera objetiva la adecuación de las soluciones propuestas, así como su viabilidad técnica, organizativa y operativa.

**En consecuencia, la propuesta cumple sobradamente los requisitos exigidos en el pliego para los criterios sujetos a juicio de valor, superando ampliamente el umbral mínimo establecido.**

## INTRODUCCIÓN Y MARCO DE REFERENCIA

### Objeto del informe

**El presente informe tiene por objeto la evaluación técnica de la propuesta presentada por empresa_s** en el procedimiento de contratación basado en el Sistema Dinámico de Adquisición SDA 26/2021, relativo a los servicios de desarrollo evolutivo, mantenimiento correctivo y mejora del ecosistema EducaMadrid.

La valoración se realiza conforme a los criterios establecidos en el Documento de Invitación, centrándose en los aspectos sujetos a juicio de valor.

### Alcance del contrato

El alcance del contrato se define a través del Anexo II del pliego, que recoge un conjunto amplio de actuaciones sobre un ecosistema tecnológico complejo, caracterizado por:

- La coexistencia de múltiples sistemas heterogéneos.

- La existencia de dependencias funcionales entre plataformas.

- La necesidad de garantizar disponibilidad continua del servicio.

- La exigencia de integrar nuevas funcionalidades sin comprometer la estabilidad del sistema.

Este contexto implica la necesidad de que las propuestas técnicas no se limiten a enumerar actuaciones, sino que desarrollen soluciones concretas que permitan su ejecución en condiciones reales.

### Metodología de evaluación

La evaluación se ha llevado a cabo mediante un análisis estructurado en dos fases.

En primer lugar, se ha analizado el contenido técnico de la propuesta, identificando el grado de desarrollo de las soluciones planteadas en cada uno de los proyectos definidos en el Anexo II.

En segundo lugar, este análisis se ha trasladado al sistema de valoración del pliego, asignando puntuaciones en función del nivel de adecuación observado.

Este enfoque permite garantizar la coherencia entre el análisis técnico y la valoración final.

### Principios de valoración aplicados

La valoración se ha realizado conforme a los siguientes principios:

- Objetividad, basada exclusivamente en el contenido de la propuesta.

- Trazabilidad, asegurando la correspondencia entre análisis y puntuación.

- Proporcionalidad, ajustando la valoración al grado real de desarrollo técnico.

- Evaluabilidad, considerando únicamente aquellos elementos que pueden ser verificados.

### Consideraciones preliminares sobre la propuesta

Desde una perspectiva global, la propuesta presentada por empresa_s muestra un alto nivel de coherencia técnica y organizativa, así como una adecuada estructuración de la información.

Se observa que:

- Los distintos apartados de la memoria mantienen una relación lógica entre sí.

- Las soluciones técnicas se presentan de manera alineada con los requisitos del pliego.

- Existe correspondencia entre la comprensión de los requisitos y las actuaciones planteadas.

Asimismo, la propuesta permite identificar con claridad la forma en que se abordarán los distintos proyectos, lo que facilita su evaluación objetiva.

## ANÁLISIS DETALLADO DE LA SOLUCIÓN TÉCNICA

### Consideraciones generales del análisis técnico

#### Enfoque global de evaluación de la propuesta

El análisis de la propuesta técnica presentada por empresa_s se ha llevado a cabo atendiendo a un criterio fundamental: la capacidad de la oferta para traducir los requisitos definidos en el Anexo II en soluciones técnicas concretas, coherentes y verificables. **Esta aproximación, plenamente alineada con el modelo de evaluación previsto en el Documento de Invitación, permite diferenciar entre propuestas meramente descriptivas y aquellas que, como la analizada, incorporan un desarrollo técnico suficientemente detallado como para garantizar su evaluabilidad.**

**A este respecto, la propuesta presenta una estructura claramente orientada a facilitar dicha evaluación, en la medida en que cada uno de los ámbitos funcionales analizados incorpora un desarrollo técnico específico, evitando enfoques genéricos o abstractos.** Esta circunstancia reviste especial relevancia desde el punto de vista de la valoración, puesto que permite establecer una relación directa entre los requisitos del pliego y las soluciones planteadas, eliminando la necesidad de realizar interpretaciones extensivas o inferencias por parte del órgano evaluador.

**De igual forma, cabe señalar que la propuesta no se limita a describir el “qué” de las actuaciones, sino que desarrolla de forma expresa el “cómo”, incorporando mecanismos, tecnologías y procedimientos que permiten verificar de manera objetiva su viabilidad.** Este nivel de concreción constituye uno de los elementos determinantes de la valoración positiva de la oferta, al situarla en un plano claramente superior en términos de evaluabilidad.

#### Nivel de desarrollo técnico y grado de concreción

Uno de los aspectos más relevantes del análisis realizado radica en el elevado grado de desarrollo técnico que presenta la propuesta. **Lejos de limitarse a una enumeración de funcionalidades o a la reproducción de los requisitos del pliego, la memoria incorpora una definición detallada de las soluciones planteadas, incluyendo tanto los componentes tecnológicos como los procesos operativos asociados.**

**En este sentido, se observa que la propuesta identifica de manera sistemática los elementos necesarios para la implementación de cada actuación, describiendo no solo las funcionalidades esperadas, sino también los mecanismos concretos mediante los cuales se llevarán a cabo.** De este modo, aspectos como la integración entre sistemas, el tratamiento de datos, la gestión de operaciones asíncronas o la aplicación de medidas de seguridad no se presentan como declaraciones genéricas, sino como soluciones técnicas desarrolladas.

**Este nivel de concreción resulta determinante desde la perspectiva de la evaluación, ya que permite verificar la adecuación de la propuesta sin necesidad de suposiciones o interpretaciones adicionales.** En consecuencia, la oferta cumple plenamente con el principio de evaluabilidad exigido en procedimientos de contratación pública, en los que la valoración debe basarse exclusivamente en elementos verificables contenidos en la documentación presentada.

#### Coherencia interna y alineación de la propuesta

**El análisis de la documentación técnica pone de manifiesto una elevada coherencia interna entre los distintos apartados de la propuesta.** Así, la arquitectura definida en los bloques iniciales se corresponde de manera consistente con las soluciones desarrolladas en los distintos sistemas, mientras que la metodología y la planificación se alinean correctamente con la naturaleza de las actuaciones descritas.

**Esta coherencia no debe entenderse como un elemento meramente formal, sino como un indicador directo de la madurez técnica de la propuesta.** La existencia de una correspondencia clara entre los distintos niveles de definición —desde la arquitectura hasta la ejecución— permite concluir que la solución ha sido concebida de manera integral, evitando inconsistencias que pudieran comprometer su viabilidad.

A efectos de valoración, este aspecto resulta especialmente relevante, en la medida en que garantiza la trazabilidad entre los distintos elementos analizados y permite fundamentar las puntuaciones asignadas en una base técnica sólida.

#### Comprensión del entorno EducaMadrid

La propuesta evidencia una comprensión detallada del entorno tecnológico y operativo en el que se enmarca el contrato, lo que se traduce en la adopción de un enfoque realista y adaptado a las características del sistema.

En particular, se identifica una correcta interpretación de la complejidad inherente a EducaMadrid, caracterizada por la coexistencia de múltiples plataformas tecnológicas, la interdependencia entre sistemas y la necesidad de garantizar la continuidad del servicio en entornos de alta concurrencia.

Esta comprensión se materializa en la adopción de criterios técnicos adecuados, entre los que destacan la intervención no intrusiva sobre sistemas existentes, la priorización de mecanismos de integración frente a modificaciones estructurales y el uso de arquitecturas desacopladas que minimizan el impacto de los cambios.

Desde el punto de vista de la evaluación, la correcta interpretación del entorno constituye un elemento esencial, ya que permite valorar la viabilidad real de las soluciones propuestas y, en consecuencia, su adecuación al objeto del contrato.

#### Arquitectura como elemento estructural de la propuesta

**Uno de los elementos diferenciales de la propuesta analizada es la incorporación de una arquitectura técnica específica para cada uno de los subproyectos definidos en el Anexo II.** Este enfoque, que trasciende la mera definición de un modelo arquitectónico general, permite adaptar las soluciones a las particularidades de cada sistema, garantizando una mayor precisión en su diseño.

**De este modo, la propuesta no se limita a establecer principios generales de arquitectura, sino que desarrolla estructuras específicas para sistemas como Aulas Virtuales, Mediateca, Cloud, sistemas de identidad o plataformas de comunicación.** Cada uno de estos ámbitos incorpora una arquitectura propia, ajustada a sus necesidades funcionales y no funcionales, lo que permite su evaluación de forma individualizada.

**Este tratamiento diferenciador refuerza la calidad técnica de la propuesta, al evidenciar un análisis pormenorizado del Anexo II y una adaptación real de las soluciones a los requisitos definidos.** Asimismo, permite al órgano evaluador verificar con mayor precisión la adecuación de cada solución, contribuyendo a la objetividad del proceso de valoración.

#### Integración de requisitos, propuestas y valor añadido

**Otro de los aspectos más relevantes de la propuesta es la estructura sistemática con la que se abordan los distintos requisitos del pliego.** En todos los ámbitos analizados, se observa un patrón recurrente consistente en la identificación del requisito, la definición de la solución propuesta, la incorporación de mejoras y la aportación de elementos de valor añadido.

Este enfoque estructural permite que cada requisito no solo sea cubierto, sino ampliado mediante propuestas adicionales orientadas a mejorar la eficiencia, la seguridad o la usabilidad del sistema. **En consecuencia, la propuesta no se limita a cumplir con lo exigido, sino que incorpora mejoras que incrementan el valor global de la solución.**

**Desde la perspectiva de la evaluación, esta metodología presenta una doble ventaja.** Por un lado, facilita la trazabilidad entre requisitos y soluciones, al permitir identificar de manera directa qué actuaciones se corresponden con cada elemento del Anexo II. Por otro lado, proporciona un valor diferencial a la oferta, en la medida en que incorpora mejoras que van más allá del mínimo exigible.

#### Evaluabilidad y fundamentación de la valoración

**Finalmente, debe destacarse que la propuesta presenta un grado de evaluabilidad elevado, en la medida en que incorpora información suficiente para verificar la adecuación de las soluciones planteadas.** Este aspecto resulta fundamental desde el punto de vista jurídico, ya que la valoración de las ofertas debe basarse en elementos objetivos y verificables, evitando cualquier tipo de arbitrariedad.

En este caso, la existencia de definiciones técnicas concretas, la identificación de tecnologías y procesos y la coherencia entre los distintos niveles de la propuesta permiten fundamentar la valoración en criterios técnicos sólidos.

En consecuencia, la puntuación asignada en los distintos subcriterios encuentra su justificación directa en el contenido de la memoria, lo que refuerza la solidez del informe y su capacidad de defensa frente a eventuales alegaciones.

### Análisis detallado por proyectos del Anexo II

#### Bloque de servicios transversales (TRA)

##### Consideración general del bloque

**El bloque de servicios transversales constituye el eje estructural del ecosistema EducaMadrid, al ser responsable de la cohesión funcional entre los distintos sistemas que lo integran.** Este bloque incluye elementos críticos como la gestión de identidades, la sincronización de usuarios, los mecanismos de integración y los servicios comunes de autenticación.

El análisis de la propuesta de empresa_s pone de manifiesto una comprensión profunda de esta naturaleza transversal, abordando el sistema no como un conjunto de actuaciones aisladas, sino como una capa coordinada que permite garantizar la coherencia del conjunto del ecosistema.

En particular, se observa que la solución propuesta:

- Establece mecanismos explícitos de integración entre sistemas.

- Define procesos de sincronización controlados y trazables.

- Incorpora medidas de seguridad alineadas con el modelo de identidad.

- Evita intervenciones intrusivas en los sistemas núcleo, priorizando la integración mediante servicios.

Este enfoque permite considerar que la propuesta aborda adecuadamente la complejidad inherente a este bloque, garantizando su evaluabilidad y viabilidad técnica.

##### Proyecto TRA1 – Sincronización de usuarios y gestión de identidades

La sincronización de usuarios constituye uno de los elementos críticos del ecosistema EducaMadrid, al garantizar la coherencia de la información entre sistemas y permitir la correcta gestión de accesos.

La propuesta de empresa_s desarrolla este aspecto mediante un enfoque estructurado basado en:

- Procesos de sincronización desacoplados.

- Ejecución mediante pipelines controlados.

- Uso de mecanismos idempotentes para evitar inconsistencias.

- Incorporación de sistemas de auditoría y trazabilidad.

Este enfoque técnico resulta especialmente adecuado en entornos distribuidos, al permitir gestionar volúmenes elevados de datos sin comprometer la integridad del sistema.

Asimismo, la propuesta contempla mecanismos de:

- Control de errores durante la sincronización.

- Reconciliación de datos entre sistemas.

- Ejecución mediante procesos en segundo plano para minimizar impacto.

La definición de estos elementos permite evaluar de manera objetiva la viabilidad de la solución, al describir de forma explícita los mecanismos de implementación.

##### Proyecto TRA2 – Integración y coherencia entre sistemas

La integración entre sistemas es uno de los elementos más complejos en entornos heterogéneos como EducaMadrid.

La propuesta aborda este aspecto mediante un modelo basado en:

- Integración mediante APIs internas versionadas.

- Definición de contratos de datos explícitos.

- Uso de mecanismos de autenticación centralizados.

Este enfoque permite:

- Garantizar la coherencia en el intercambio de información.

- Reducir el acoplamiento entre sistemas.

- Facilitar la evolución independiente de los componentes.

Además, la propuesta incorpora mecanismos de control de integración, lo que permite verificar el correcto funcionamiento de los flujos de información.

En consecuencia, la solución resulta evaluable y adecuada para el entorno definido en el pliego.

##### Proyecto TRA3 – Seguridad transversal

En materia de seguridad, la propuesta incorpora un enfoque claramente estructurado, integrando los mecanismos de control desde el diseño de la solución.

Se identifican elementos como:

- Autenticación centralizada mediante SSO.

- Uso de LDAP seguro.

- Aplicación del principio de mínimo privilegio.

- Protección frente a vulnerabilidades habituales.

- Registro y auditoría de operaciones.

Este enfoque permite garantizar que la seguridad no se limita a un conjunto de medidas aisladas, sino que forma parte integral de la arquitectura.

La definición detallada de estos mecanismos permite evaluar su adecuación al entorno y verificar el cumplimiento de los requisitos del pliego.

### Conclusión del bloque TRA

El bloque de servicios transversales presenta un alto nivel de desarrollo técnico, con una definición clara de los mecanismos de sincronización, integración y seguridad.

La propuesta permite evaluar de forma objetiva:

- La coherencia del modelo de datos.

- La viabilidad de los procesos de integración.

- La capacidad de mantener la integridad del sistema.

En consecuencia, el bloque se considera adecuadamente resuelto, mostrando un alto grado de alineación con los requisitos del Anexo II.

#### Bloque de Aulas Virtuales (AV – Moodle)

##### Consideración general del bloque

El sistema de Aulas Virtuales constituye uno de los componentes más críticos del ecosistema EducaMadrid, tanto por el volumen de usuarios como por su impacto directo en la actividad docente.

Desde el punto de vista técnico, este sistema requiere soluciones específicas orientadas a:

- La gestión de alta concurrencia.

- La optimización del rendimiento.

- La integración con otros servicios.

- La evolución controlada del sistema.

La propuesta de empresa_s aborda este bloque mediante una arquitectura claramente definida, basada en:

- Sistemas multinodo con balanceo de carga.

- Uso de mecanismos de caché distribuidos.

- Procesamiento asíncrono de tareas intensivas.

- Separación entre operaciones interactivas y procesos en segundo plano.

Este enfoque permite adaptar la solución a un entorno real de uso intensivo, garantizando su evaluabilidad.

##### Proyecto AV1 – Evolución y mantenimiento del core

La propuesta desarrolla de manera adecuada los procesos de evolución del core de Moodle, incorporando mecanismos orientados a garantizar la estabilidad del sistema.

En particular, se observa:

- Definición de procesos de actualización controlada.

- Uso de entornos diferenciados para validación previa.

- Aplicación de pruebas de regresión.

- Gestión de compatibilidad con plugins existentes.

Este enfoque permite minimizar el riesgo asociado a la evolución del sistema, garantizando que las actualizaciones no afectan a la operativa.

La existencia de estos mecanismos permite evaluar de forma objetiva la viabilidad de las actuaciones propuestas.

##### Proyecto AV4 – Rendimiento y optimización

En el ámbito del rendimiento, la propuesta presenta un nivel de desarrollo técnico elevado.

Se identifican medidas concretas como:

- Optimización de consultas SQL.

- Uso de sistemas de caché (como Redis o equivalentes).

- Distribución de carga entre frontend y procesos batch.

- Monitorización de rendimiento y detección de cuellos de botella.

Asimismo, se contempla la ejecución de pruebas de carga, lo que permite validar el comportamiento del sistema en escenarios reales.

Este nivel de detalle permite evaluar la adecuación de la solución al requisito de rendimiento.

##### Proyecto AV11 – Funcionalidades avanzadas y procesamiento intensivo

En relación con funcionalidades avanzadas, como la incorporación de inteligencia artificial o procesos de generación de contenidos, la propuesta adopta un enfoque técnicamente adecuado.

En particular:

- Se define el uso de procesamiento asíncrono.

- Se separan las cargas intensivas del flujo de usuario.

- Se utilizan colas de procesamiento para gestionar tareas de larga duración.

Este planteamiento permite evitar impactos negativos en el rendimiento del sistema, manteniendo la experiencia de usuario.

La definición de estos mecanismos permite evaluar su adecuación de forma objetiva.

##### Integración con otros sistemas

El sistema de Aulas Virtuales se integra con múltiples componentes del ecosistema.

La propuesta desarrolla estos aspectos mediante:

- Integración con sistemas de identidad (SSO, LDAP).

- Conexión con plataformas de contenidos.

- Interoperabilidad con sistemas Cloud y Mediateca.

La definición de estas integraciones permite evaluar su coherencia dentro del sistema global.

##### Escalabilidad y continuidad del servicio

Uno de los aspectos mejor desarrollados de la propuesta es la consideración de la escalabilidad.

Se incorporan mecanismos como:

- Balanceo de carga entre nodos.

- Distribución de recursos.

- Separación de servicios críticos.

- Monitorización continua.

Estos elementos permiten garantizar el funcionamiento del sistema en escenarios de alta concurrencia.

### Conclusión del bloque AV

El bloque de Aulas Virtuales presenta un alto nivel de desarrollo técnico, con soluciones específicas para los principales retos del sistema.

La propuesta permite evaluar de forma objetiva:

- El comportamiento del sistema en carga.

- La viabilidad de las actualizaciones.

- La integración con otros componentes.

En consecuencia, el bloque se considera adecuadamente resuelto, con un alto grado de adecuación al Anexo II.

#### Bloque de Mediateca (MED)

##### Consideración general del bloque

El sistema de Mediateca constituye uno de los componentes estratégicos del ecosistema EducaMadrid, al actuar como repositorio centralizado de contenidos multimedia utilizados en la actividad educativa.

Desde el punto de vista técnico, este sistema presenta una complejidad elevada, derivada de:

- El volumen de datos almacenados

- La necesidad de procesar contenidos multimedia

- La exigencia de permitir búsquedas eficientes

- La integración con múltiples plataformas

La propuesta presentada aborda este bloque mediante un enfoque arquitectónico específico, orientado a la gestión eficiente del contenido multimedia y a la optimización del acceso a la información.

En particular, se observa que la solución se apoya en:

- Procesamiento asíncrono para operaciones intensivas

- Indexación especializada de metadatos

- Separación entre almacenamiento y consulta

- Control de visibilidad por usuario y contexto

Este enfoque permite evaluar de forma objetiva la capacidad del sistema para gestionar cargas elevadas y garantizar tiempos de respuesta adecuados.

##### Proyecto MED1 – Procesamiento multimedia y generación de contenidos

La propuesta desarrolla con un alto nivel de detalle los procesos asociados al tratamiento de contenidos multimedia.

En particular, se identifican mecanismos como:

- Uso de colas de procesamiento para tareas intensivas

- Ejecución diferida de procesos de transcodificación

- Generación automática de subtítulos y resúmenes

- Separación entre procesamiento y uso interactivo

Este enfoque técnico resulta especialmente adecuado en entornos con gran volumen de datos, ya que evita impactar negativamente en la experiencia de usuario.

Asimismo, la incorporación de procesamiento asíncrono permite dimensionar adecuadamente la infraestructura, optimizando el uso de recursos.

La definición explícita de estos elementos permite evaluar la viabilidad de la solución de manera objetiva.

##### Proyecto MED2 – Búsqueda e indexación de contenidos

El sistema de búsqueda constituye uno de los elementos clave de la Mediateca.

La propuesta aborda este aspecto mediante:

- Uso de motores de indexación especializados

- Normalización de metadatos

- Indexación incremental de contenidos

- Definición de filtros avanzados de consulta

Este enfoque permite:

- Reducir los tiempos de respuesta en búsquedas complejas

- Mejorar la experiencia de usuario

- Gestionar grandes volúmenes de contenido sin degradación

Asimismo, se contempla la posibilidad de reindexación controlada, lo que permite mantener la coherencia del sistema a lo largo del tiempo.

La existencia de estos mecanismos permite evaluar adecuadamente la solución en términos de rendimiento y escalabilidad.

##### Arquitectura de almacenamiento y distribución

La propuesta define un modelo de almacenamiento orientado a alta disponibilidad y rendimiento.

Se identifican elementos como:

- Uso de almacenamiento distribuido

- Separación entre contenido y metadatos

- Control de acceso basado en roles

- Optimización del acceso a contenidos

Este enfoque permite:

- Garantizar la disponibilidad del sistema

- Facilitar la escalabilidad

- Mejorar la eficiencia en el acceso a la información

En consecuencia, la arquitectura planteada resulta adecuada para el contexto del sistema.

##### Integración con el ecosistema EducaMadrid

El sistema de Mediateca no se plantea de forma aislada, sino integrado dentro del ecosistema.

La propuesta define mecanismos de integración con:

- Aulas Virtuales

- Sistemas Cloud

- Plataformas de identidad

- Servicios de contenido

Esta integración se realiza mediante:

- APIs internas

- Mecanismos de autenticación compartidos

- Control de acceso unificado

Este enfoque permite garantizar la coherencia del sistema y facilita su uso en distintos contextos.

### Conclusión del bloque MED

El bloque de Mediateca presenta un alto nivel de desarrollo técnico, con soluciones específicas para los principales retos del sistema.

La propuesta permite evaluar de forma objetiva:

- La gestión de contenidos multimedia

- La eficiencia del sistema de búsqueda

- La integración con el resto del ecosistema

En consecuencia, se considera que el bloque se encuentra adecuadamente resuelto, con un elevado grado de adecuación al pliego.

#### Bloque de Correo Electrónico (COR)

##### Consideración general del bloque

El sistema de correo electrónico constituye un servicio fundamental dentro del ecosistema EducaMadrid, tanto por su uso intensivo como por su papel como herramienta de comunicación institucional.

Desde el punto de vista técnico, este sistema requiere:

- Alta disponibilidad

- Gestión eficiente de grandes volúmenes de datos

- Integración con sistemas de identidad

- Seguridad en las comunicaciones

La propuesta aborda estos requisitos mediante un enfoque técnico estructurado, basado en la modernización del sistema existente y la optimización de su funcionamiento.

##### Proyecto COR1 – Evolución y mantenimiento del sistema

La propuesta contempla actuaciones de evolución orientadas a garantizar la estabilidad y mejora del sistema de correo.

En particular, se observa:

- Actualización controlada del entorno Roundcube

- Optimización de la gestión de buzones

- Racionalización de plugins

- Mejora del rendimiento en operaciones frecuentes

Este nivel de desarrollo permite evaluar la capacidad del sistema para operar en condiciones de carga.

##### Proyecto COR2 – Funcionalidades avanzadas y asistencia al usuario

En relación con la incorporación de funcionalidades avanzadas, la propuesta plantea un enfoque equilibrado, orientado a mejorar la experiencia del usuario sin comprometer la seguridad.

Se identifican actuaciones como:

- Sistemas de asistencia en redacción

- Resumen automático de mensajes

- Mejora de la usabilidad del sistema

Estos elementos se integran manteniendo la seguridad de la información, evitando la exposición innecesaria de datos.

##### Seguridad e integración

La propuesta define mecanismos de seguridad adecuados, incluyendo:

- Uso de protocolos seguros de comunicación

- Integración con sistemas de autenticación

- Protección frente a amenazas habituales

- Control de acceso a la información

Asimismo, se contempla la integración con el resto de sistemas del ecosistema, lo que permite una gestión coherente de los usuarios.

### Conclusión del bloque COR

El bloque de correo electrónico presenta un nivel adecuado de desarrollo técnico, con soluciones orientadas a mejorar el rendimiento, la seguridad y la integración del sistema.

La propuesta permite evaluar de forma objetiva la viabilidad de las actuaciones planteadas.

#### Bloque Cloud (CLO – Nextcloud / Collabora)

##### Consideración general del bloque

El sistema Cloud constituye un elemento clave en la gestión de almacenamiento y colaboración dentro del ecosistema EducaMadrid.

Desde el punto de vista técnico, este sistema requiere:

- Gestión eficiente del almacenamiento

- Sincronización entre dispositivos

- Edición colaborativa

- Integración con otros sistemas

La propuesta aborda estos aspectos mediante un modelo claramente definido, orientado a garantizar la eficiencia y la escalabilidad.

##### Proyecto CLO1 – Gestión de almacenamiento y sincronización

La propuesta define mecanismos específicos para la gestión de almacenamiento, incluyendo:

- Uso de sistemas de almacenamiento escalable

- Control de versiones de archivos

- Gestión de conflictos en sincronización

- Optimización del acceso a datos

Estos elementos permiten evaluar la viabilidad de la solución en entornos colaborativos.

##### Proyecto CLO2 – Edición colaborativa

En relación con la edición colaborativa, la propuesta contempla:

- Integración con herramientas específicas

- Control de concurrencia en edición

- Gestión de versiones

- Recuperación ante conflictos

Este enfoque permite garantizar la consistencia del sistema en escenarios de uso intensivo.

##### Arquitectura e integración

La arquitectura propuesta incorpora:

- Separación de componentes

- Integración mediante APIs

- Uso de sistemas de caché

- Control de accesos

Asimismo, se contempla la integración con:

- Sistemas de identidad

- Aulas Virtuales

- Plataformas de contenido

Esto permite garantizar la coherencia del sistema dentro del ecosistema global.

### Conclusión del bloque CLO

El bloque Cloud presenta un nivel elevado de desarrollo técnico, con soluciones completas para la gestión de almacenamiento y colaboración.

La propuesta permite evaluar de forma objetiva:

- La viabilidad de la sincronización

- La consistencia de la edición colaborativa

- La integración con el ecosistema

#### Bloque de Videoconferencia (VID)

##### Consideración general del bloque

El sistema de videoconferencia constituye un componente de especial criticidad dentro del ecosistema EducaMadrid, al estar directamente vinculado a la actividad docente en tiempo real y a la prestación de servicios síncronos de alta demanda.

Desde el punto de vista técnico, este tipo de sistemas presenta requisitos especialmente exigentes en términos de:

- Consumo de recursos

- Gestión de concurrencia

- Distribución de carga

- Garantía de calidad de servicio

La propuesta presentada por empresa_s aborda este bloque mediante una arquitectura especializada, basada en la coexistencia de distintas soluciones tecnológicas adaptadas a los distintos escenarios de uso.

En particular, se observa que la solución contempla:

- Distribución de servicios en función del tipo de sesión

- Uso de nodos dedicados para procesamiento multimedia

- Integración con plataformas ya existentes en el ecosistema

- Monitorización de calidad de servicio

Este enfoque permite adaptar la solución a la heterogeneidad de los casos de uso, lo que resulta adecuado en un entorno con alta variabilidad de carga.

##### Proyecto VID1 – Integración con plataformas educativas

La propuesta desarrolla de forma adecuada la integración del sistema de videoconferencia con el resto de plataformas de EducaMadrid.

Se identifican mecanismos como:

- Integración con Aulas Virtuales

- Conexión con sistemas de identidad

- Gestión unificada de accesos

- Sincronización de sesiones con otros servicios

Este modelo permite:

- Facilitar el acceso de los usuarios

- Garantizar la coherencia de las sesiones

- Reducir la complejidad operativa

Asimismo, la definición de estos mecanismos permite evaluar de manera objetiva la adecuación de la solución.

##### Proyecto VID2 – Rendimiento, escalabilidad y calidad de servicio

Desde el punto de vista del rendimiento, la propuesta incorpora un conjunto de medidas específicas orientadas a garantizar la estabilidad del sistema.

En particular, se observan:

- Mecanismos de balanceo de carga

- Distribución dinámica de recursos

- Control de admisión de sesiones

- Monitorización de parámetros de calidad

Este enfoque permite:

- Mantener la calidad de servicio en condiciones de alta carga

- Evitar degradaciones en audio y vídeo

- Priorizar sesiones críticas cuando sea necesario

La definición de estos elementos permite evaluar la capacidad del sistema para operar en escenarios reales de uso intensivo.

### Conclusión del bloque VID

El bloque de videoconferencia presenta un desarrollo técnico adecuado, con una arquitectura específica orientada a garantizar la estabilidad y el rendimiento del sistema.

La propuesta permite evaluar de forma objetiva:

- La integración con el ecosistema

- La capacidad de gestión de carga

- La calidad de servicio

En consecuencia, se considera que el bloque se encuentra correctamente resuelto y alineado con los requisitos del pliego.

#### a 2.2.14 Bloques adicionales y sistemas complementarios

##### Consideración general del conjunto

El conjunto de sistemas adicionales definidos en el Anexo II representa una parte sustancial del alcance del contrato, tanto por su número como por su diversidad funcional.

Estos sistemas incluyen, entre otros:

- Portal CAU

- Sistemas de gestión funcional (Seguimientos, Albor, EducaSaac, AbiesWeb)

- Sistemas de comunicación (Avisos, Boletines, Foros)

- Sistemas de soporte y analítica

- Componentes auxiliares y aplicaciones específicas

A diferencia de propuestas en las que estos sistemas se abordan de manera superficial, en el caso analizado se observa un tratamiento homogéneo y estructurado, alineado con el modelo arquitectónico general definido para el conjunto de la solución.

##### Enfoque técnico aplicado a los sistemas complementarios

La propuesta adopta un enfoque basado en la proporcionalidad técnica, adaptando la complejidad de la solución al nivel de criticidad de cada sistema.

En este sentido:

- Los sistemas críticos presentan mayor nivel de detalle y desarrollo técnico.

- Los sistemas de menor complejidad se abordan con soluciones adecuadas a su funcionamiento.

- Todos los sistemas comparten principios comunes de arquitectura, seguridad y control.

Este enfoque permite:

- Evitar sobredimensionamiento innecesario

- Mantener coherencia global

- Garantizar la mantenibilidad del sistema

##### Desarrollo técnico de los sistemas específicos

El análisis de los distintos sistemas complementarios permite identificar que, de manera general:

- Se definen arquitecturas funcionales mantenibles

- Se contemplan mecanismos de actualización tecnológica

- Se incorporan medidas de seguridad y control de accesos

- Se optimizan consultas y procesos de datos

- Se establecen procedimientos de despliegue controlado

Asimismo, se observa que la propuesta:

- Evita enfoques genéricos

- Adapta las soluciones a cada sistema

- Mantiene la coherencia con la arquitectura global

Este nivel de definición permite evaluar la viabilidad de las actuaciones de manera objetiva.

##### Sistemas de soporte, comunicación y analítica

En relación con sistemas como:

- Portal CAU

- Avisos

- Boletines

- Seguimientos

la propuesta presenta soluciones orientadas a:

- La gestión eficiente de la información

- La mejora de la trazabilidad

- La optimización de consultas

- La integración con el resto del ecosistema

Se observa además la incorporación de:

- Interfaces web estructuradas

- Sistemas de control de permisos

- Mecanismos de registro de operaciones

Esto permite garantizar una adecuada operativa del sistema.

##### Sistemas funcionales especializados

En sistemas como:

- Albor

- EducaSaac

- AbiesWeb

- Aplicaciones específicas

la propuesta adopta un enfoque basado en:

- Modernización progresiva

- Optimización del rendimiento

- Mejora de la mantenibilidad

- Protección de datos

Este planteamiento resulta especialmente adecuado en sistemas con componentes legacy, ya que permite su evolución sin introducir riesgos innecesarios.

##### Coherencia global del conjunto

Uno de los elementos más relevantes del análisis es la coherencia global del conjunto de sistemas complementarios.

Se observa que:

- Todos los sistemas comparten principios arquitectónicos comunes

- Se mantienen criterios homogéneos de seguridad

- La integración con sistemas transversales está definida

- La gestión de usuarios es consistente

Este enfoque evita la fragmentación del sistema y facilita su evolución conjunta.

##### Evaluabilidad del bloque complementario

El nivel de detalle presentado en este conjunto de sistemas permite:

- Identificar las soluciones técnicas aplicadas

- Evaluar su adecuación al pliego

- Verificar su viabilidad en entorno real

En consecuencia, el bloque resulta plenamente evaluable, contribuyendo de manera significativa a la valoración global de la propuesta.

### Conclusión del bloque complementario

El conjunto de sistemas adicionales presenta un tratamiento técnico homogéneo, adaptado a la naturaleza de cada aplicativo y alineado con la arquitectura global de la solución.

La propuesta permite evaluar de forma objetiva:

- El grado de desarrollo técnico de cada sistema

- Su integración en el ecosistema

- Su viabilidad operativa

## CONCLUSIÓN DEL CAPÍTULO 2

El análisis detallado de la solución técnica presentada por empresa_s pone de manifiesto un alto grado de desarrollo en todos los bloques funcionales definidos en el Anexo II.

De forma global, se concluye que la propuesta:

- Define soluciones técnicas concretas y coherentes

- Mantiene alineación con la arquitectura real del sistema

- Incorpora mecanismos de integración, control y validación

- Permite evaluar de manera objetiva la viabilidad de las actuaciones

Asimismo:

- No se identifican errores conceptuales relevantes

- No existen carencias estructurales en la definición de la solución

- Se garantiza la coherencia entre sistemas

- Se acredita la capacidad de ejecución en entorno real

En consecuencia, el conjunto de la solución técnica se sitúa en un nivel alto de adecuación, proporcionando una base sólida para la valoración de los criterios sujetos a juicio de valor.

## VALORACIÓN DE LA PROPUESTA

### Fundamentación de la valoración

**La valoración técnica de la propuesta presentada por empresa_s** se ha llevado a cabo conforme a los criterios definidos en el Documento de Invitación, tomando como base el análisis técnico exhaustivo desarrollado en el Capítulo 2 del presente informe.

Este enfoque metodológico garantiza que la puntuación asignada es el resultado directo de un proceso de evaluación objetivo, trazable y verificable, basado exclusivamente en el contenido de la memoria técnica aportada por el licitador.

En este sentido, la valoración no responde a apreciaciones generales o subjetivas, sino a la constatación del grado de adecuación de la propuesta a los requisitos definidos, atendiendo a:

- El nivel de desarrollo técnico de las soluciones planteadas

- La coherencia interna de la propuesta

- La capacidad de ejecución en un entorno real

- La evaluabilidad de las actuaciones descritas

Asimismo, el sistema de valoración previsto en el pliego permite la asignación de puntuaciones dentro de rangos continuos, lo que implica que la puntuación final responde a un juicio técnico global, sustentado en la evidencia contenida en la propuesta.

### Valoración de la solución técnica ofertada

#### Arquitectura de la solución

La propuesta presenta una arquitectura técnica claramente definida, basada en principios de modularidad, desacoplamiento y control de operaciones.

Se observa que la arquitectura:

- Se adapta a la heterogeneidad del entorno

- Define capas funcionales diferenciadas

- Establece mecanismos de integración mediante APIs y servicios

- Incorpora medidas de seguridad y control desde el diseño

A diferencia de enfoques genéricos, en este caso la arquitectura se desarrolla de forma específica para los sistemas incluidos en el Anexo II, permitiendo identificar de manera clara los mecanismos de funcionamiento.

Asimismo, la coherencia entre la arquitectura planteada y las soluciones descritas en los distintos bloques analizados permite verificar su aplicabilidad real.

En consecuencia, la arquitectura se considera:

- Adecuadamente definida

- Coherente con el entorno

- Técnicamente viable

**Puntuación: 1,80 sobre 2**

#### Comprensión de los requisitos

La propuesta acredita un alto grado de comprensión de los requisitos definidos en el pliego.

Este aspecto se evidencia en:

- La identificación de las relaciones entre sistemas

- La consideración de condicionantes operativos reales

- La integración de requisitos funcionales y no funcionales

- La adaptación de las soluciones al contexto específico

Además, la comprensión no se limita a una lectura descriptiva, sino que se traduce en soluciones técnicas concretas, lo que permite verificar su adecuación.

No se detectan errores conceptuales ni interpretaciones incorrectas de los requisitos, lo que refuerza la solidez de la propuesta.

**Puntuación: 1,90 sobre 2**

#### Viabilidad de la propuesta

La propuesta presenta un alto grado de viabilidad, tanto desde el punto de vista técnico como operativo.

En particular:

- Se definen mecanismos de despliegue controlado

- Se contemplan procesos de validación previa

- Se incorporan estrategias de rollback

- Se consideran las condiciones del entorno productivo

Asimismo, la propuesta:

- Integra la gestión de múltiples líneas de trabajo

- Considera la simultaneidad de actuaciones

- Define mecanismos de control del riesgo

Este nivel de desarrollo permite concluir que la solución es plenamente ejecutable en el contexto del contrato.

La puntuación no alcanza el máximo absoluto al considerar que, si bien la viabilidad está claramente acreditada, algunos aspectos de dimensionamiento podrían desarrollarse de manera aún más cuantificada.

**Puntuación: 0,90 sobre 1**

#### Metodología

La propuesta desarrolla la metodología de trabajo con un nivel elevado de detalle, basándose en Métrica v3 e incorporando prácticas actuales de desarrollo.

Se observa:

- Definición completa del ciclo de vida del desarrollo

- Incorporación de pruebas automatizadas

- Uso de integración continua

- Adaptación metodológica al tipo de actuación

Este enfoque permite garantizar:

- La trazabilidad de los procesos

- La calidad de los entregables

- La eficiencia en la ejecución

La metodología se presenta como plenamente aplicable al contrato, evitando tanto la rigidez como la falta de control.

**Puntuación: 0,90 sobre 1**

#### Rendimiento

El tratamiento del rendimiento en la propuesta es uno de los aspectos mejor desarrollados.

Se identifican:

- Medidas concretas de optimización

- Uso de caché y procesamiento asíncrono

- Control de concurrencia

- Definición de pruebas de carga

Además, se aborda el rendimiento de forma específica en cada sistema, lo que permite evaluar su adecuación de manera precisa.

La propuesta permite verificar que las soluciones están diseñadas para operar en condiciones reales de carga.

**Puntuación: 0,90 sobre 1**

#### Satisfacción de los requisitos del Anexo II

El análisis desarrollado en el Capítulo 2 pone de manifiesto un alto grado de satisfacción de los requisitos definidos en el Anexo II.

En particular:

- Se identifican soluciones para la totalidad de los sistemas

- Se desarrollan técnicamente las actuaciones propuestas

- Se definen mecanismos de integración y control

- Se da respuesta a los requisitos funcionales y no funcionales

No se han identificado omisiones relevantes ni carencias estructurales que impidan verificar el cumplimiento de los requisitos.

Asimismo, la propuesta presenta:

- Coherencia entre requisitos y soluciones

- Adecuación al entorno real

- Capacidad de ejecución acreditada

En consecuencia, este subcriterio se sitúa en niveles altos del baremo.

**Puntuación: 7,20 sobre 8**

#### Resultado del bloque de solución técnica

**Total solución técnica: 13,60 puntos sobre 15**

### Valoración de la planificación del servicio

#### Calendario y planificación

La propuesta presenta una planificación estructurada, alineada con el calendario educativo y orientada a minimizar el impacto sobre la operación del servicio. **No obstante, se observa un enfoque predominantemente estructurado, con menor concreción en la gestión iterativa del desarrollo evolutivo.**

Se observa:

- Definición de fases y hitos

- Secuenciación lógica de actuaciones

- Identificación de dependencias

- Inclusión de Gantt con leyenda

Este nivel de detalle permite evaluar de manera objetiva la planificación.

**Puntuación: 9,60 sobre 11**

#### Análisis de riesgos

Se incluye un análisis sistemático de riesgos con identificación, clasificación y medidas de mitigación, si bien con carácter parcialmente genérico, que incluye:

- Identificación de escenarios de riesgo

- Evaluación de impacto y probabilidad

Este enfoque permite garantizar la estabilidad del servicio.

**Puntuación: 0,85 sobre 1**

#### Plan de gestión de las Contingencias

Se definen mecanismos adecuados de continuidad del servicio, incluyendo recuperación ante fallos y despliegues controlados, alineados con entornos críticos. Estos procedimientos incluyen:

- Definición de medidas de mitigación

- Planes de contingencia

Este enfoque permite garantizar la viabilidad del servicio.

**Puntuación: 0,***90 sobre 1**

#### Plan de calidad y control

Se observa un modelo de calidad basado en:

- Indicadores de rendimiento

- Procesos de mejora continua

- Auditorías periódicas

- Control de incidencias

Esto permite asegurar la calidad del servicio durante toda la ejecución.

**Puntuación: 0,***90 sobre 1**

#### Trazabilidad del servicio

La propuesta incorpora mecanismos de trazabilidad completos, que permiten:

- Relacionar requisitos con entregables

- Registrar actuaciones

- Facilitar auditorías

**Puntuación: 0,85 sobre 1**

#### Resultado del bloque de planificación

**Total planificación: 13,10 puntos sobre 15**

### Resultado final de la valoración

Como resultado de la evaluación:

- Solución técnica: 13,60

- Planificación: 13,10

**Total: 26,70 puntos sobre 30**

## CONCLUSIONES FINALES Y JUSTIFICACIÓN DE LA VALORACIÓN

### Alcance y naturaleza de la valoración realizada

La valoración técnica recogida en el presente informe se ha realizado en estricta aplicación de lo previsto en el Documento de Invitación, y en particular conforme a los criterios sujetos a juicio de valor definidos en su apartado correspondiente.

**El análisis se ha limitado exclusivamente al contenido de la memoria técnica presentada por la empresa empresa_s, sin incorporar inferencias, presunciones de ejecución futura ni referencias a capacidades no explicitadas en la documentación aportada.** Este criterio responde al principio de objetividad que rige los procedimientos de contratación pública, conforme al cual la evaluación de las ofertas debe fundamentarse en elementos verificables contenidos en las mismas.

Asimismo, se ha aplicado de forma uniforme el mismo enfoque metodológico utilizado en la evaluación del resto de ofertas del procedimiento, garantizando así el cumplimiento del principio de igualdad de trato entre licitadores.

En consecuencia, la valoración no incorpora elementos discrecionales ajenos al contenido de la propuesta, sino que se sustenta en el análisis técnico desarrollado en el Capítulo 2, garantizando la trazabilidad completa entre requisitos, soluciones y puntuación asignada.

### Síntesis técnica de la evaluación

El análisis detallado de la propuesta permite concluir que la oferta presentada por empresa_s presenta un alto grado de desarrollo técnico, caracterizado por:

- La definición de soluciones concretas para la totalidad de los sistemas incluidos en el Anexo II.

- La existencia de arquitecturas específicas adaptadas a cada uno de los subproyectos, lo que permite un tratamiento individualizado de sus particularidades técnicas.

- La incorporación sistemática de una estructura basada en la identificación del requisito, el desarrollo de la solución, la inclusión de mejoras y la aportación de valor añadido.

- La ausencia de errores conceptuales o interpretaciones incorrectas del entorno tecnológico.

- La adecuada integración de requisitos funcionales y no funcionales dentro de una solución coherente.

Estos elementos permiten afirmar que la propuesta no se limita a cubrir el alcance del contrato, sino que desarrolla de manera efectiva las soluciones necesarias para su ejecución, incorporando además mejoras que incrementan su valor técnico.

Desde la perspectiva de la evaluación, este nivel de desarrollo permite verificar de manera objetiva la adecuación de la propuesta, eliminando la necesidad de interpretaciones adicionales y reforzando la solidez de la valoración.

### Evaluabilidad de la propuesta como elemento determinante

Uno de los aspectos clave que fundamentan la puntuación asignada es el elevado grado de evaluabilidad de la propuesta.

En procedimientos sujetos a juicio de valor, la evaluación debe basarse en la capacidad de la oferta para proporcionar información suficiente que permita verificar la adecuación de las soluciones planteadas. En este sentido, la propuesta de empresa_s presenta:

- Definiciones técnicas suficientemente detalladas.

- Identificación expresa de mecanismos de implementación.

- Descripción de procesos operativos y de integración.

- Desarrollo específico de cada sistema.

Este nivel de detalle permite comprobar cómo se ejecutarán las actuaciones descritas, lo que constituye un requisito esencial para una valoración objetiva.

A diferencia de propuestas que se limitan a descripciones generales, en este caso la información aportada permite evaluar directamente la viabilidad técnica de la solución, lo que justifica una valoración elevada dentro del baremo.

### Coherencia entre análisis técnico y puntuación asignada

La puntuación final obtenida por la propuesta no responde a un juicio global abstracto, sino al resultado directo del análisis técnico desarrollado proyecto a proyecto.

En particular, debe destacarse que:

- Los subcriterios mejor valorados coinciden con aquellos en los que la propuesta presenta mayor nivel de desarrollo técnico, como la satisfacción de requisitos y la arquitectura de la solución.

- La puntuación asignada se sitúa dentro de los rangos superiores del baremo, pero sin alcanzar en todos los casos los valores máximos, lo que refleja la aplicación del principio de proporcionalidad.

- Las puntuaciones parciales responden a una valoración diferenciada de cada aspecto, evitando incrementos automáticos o generalizados.

Este enfoque permite garantizar que la puntuación final refleja de manera fiel el contenido de la propuesta, evitando tanto la infra‑valoración como la sobrevaloración injustificada.

Asimismo, la coherencia entre el análisis técnico y la puntuación asignada refuerza la solidez del informe frente a posibles alegaciones, al permitir trazar de forma clara la relación entre las soluciones descritas y la valoración obtenida.

### Aplicación del principio de proporcionalidad

La valoración realizada se ajusta al principio de proporcionalidad, en la medida en que la puntuación asignada se corresponde con el nivel real de desarrollo técnico de la propuesta.

En este sentido, la elevada puntuación obtenida no deriva de una apreciación general favorable, sino de la constatación de que la propuesta:

- Define soluciones concretas y evaluables.

- Se adapta al entorno real del contrato.

- Incorpora mejoras que superan los mínimos exigidos.

- Mantiene coherencia interna entre todos sus componentes.

Asimismo, el hecho de que la puntuación no alcance el valor máximo absoluto en todos los subcriterios evidencia que la evaluación se ha realizado de manera crítica y discriminada, evitando una asignación automática de la máxima puntuación.

Este equilibrio entre reconocimiento de la calidad técnica y aplicación rigurosa del baremo refuerza la defensa de la valoración frente a posibles recursos.

### Justificación específica de la alta puntuación obtenida

La puntuación final de la propuesta, situada en 26,70 puntos sobre un máximo de 30, se fundamenta en una combinación de elementos técnicos que, en su conjunto, justifican su posicionamiento en el tramo alto del baremo.

Entre estos elementos destacan especialmente:

- La definición de una arquitectura específica para cada subproyecto, lo que permite una adaptación precisa a las necesidades técnicas de cada sistema.

- La estructura sistemática en la exposición de contenidos, basada en requisitos, soluciones, mejoras y valor añadido, que incrementa la claridad y profundidad de la propuesta.

- La capacidad de integrar múltiples sistemas dentro de un modelo coherente, sin generar inconsistencias.

- La incorporación de mecanismos de control, validación y trazabilidad que permiten verificar la ejecución de las actuaciones.

Este conjunto de factores no solo permite cumplir los requisitos del pliego, sino que aporta valor diferencial a la propuesta, lo que justifica su posicionamiento en niveles altos de la valoración.

### Ausencia de elementos que limiten la valoración

El análisis técnico realizado no ha identificado elementos que limiten de forma significativa la valoración de la propuesta.

En particular:

- No se detectan omisiones relevantes de requisitos.

- No se identifican errores conceptuales en la interpretación del sistema.

- No existen carencias estructurales que impidan evaluar la viabilidad de la solución.

Esta ausencia de deficiencias relevantes constituye un factor determinante en la puntuación obtenida, ya que elimina los elementos habitualmente asociados a reducciones significativas en la valoración.

### Garantía de igualdad de trato y objetividad

La evaluación realizada garantiza el cumplimiento de los principios de igualdad de trato, transparencia y objetividad.

En particular:

- Se ha aplicado el mismo esquema de análisis utilizado para el resto de ofertas.

- Se han utilizado criterios homogéneos en la valoración de los distintos bloques.

- Se ha evitado la introducción de elementos no previstos en el pliego.

Este enfoque permite descartar cualquier posibilidad de tratamiento desigual, asegurando que la puntuación asignada responde exclusivamente a la calidad técnica de la propuesta.

### Conclusión final del informe

**A la vista del análisis técnico realizado, se concluye que la propuesta presentada por empresa_s** presenta un alto grado de adecuación a los requisitos del contrato, tanto en términos de desarrollo técnico como de viabilidad de las soluciones planteadas.

La puntuación obtenida, de **26,70 puntos** sobre un máximo de 30, refleja de manera fiel el nivel de calidad de la propuesta, situándola en los tramos superiores del baremo de valoración.

Esta puntuación se fundamenta en elementos objetivos y verificables, derivados directamente del contenido de la memoria técnica, lo que garantiza la solidez del informe y su capacidad de defensa frente a cualquier posible alegación.

En consecuencia, puede afirmarse que la propuesta cumple plenamente los requisitos establecidos en el pliego para los criterios sujetos a juicio de valor, ofreciendo una solución técnica robusta, coherente y viable para la ejecución del contrato.

## ANEXO RELACIÓN DE PROYECTOS Y GRADO DE DESARROLLO EN LA PROPUESTA TÉCNICA DE empresa_s

El presente anexo identifica, de manera sistemática, el grado de desarrollo técnico de los proyectos y subproyectos incluidos en el Anexo II del pliego, conforme a su tratamiento en la memoria técnica presentada por empresa_s

La clasificación utilizada distingue los siguientes niveles:

- **Propuesta técnica incluida (desarrollo suficiente)** → existe una propuesta técnica concreta, con definición de arquitectura, procesos o mecanismos de implementación

- **Propuesta técnica incluida (desarrollo deficiente***)** → existe una propuesta técnica pero su definición es genérica, sin procesos o mecanismos de implementación concretos.

- **Propuesta técnica con valor añadido** → cuando el informe identifica mejoras más allá de los requisitos incluidos en el pliego.

Debe destacarse que, a diferencia de otras propuestas analizadas, la de empresa_s presenta un enfoque sistemático basado en el **desarrollo específico por proyecto y subproyecto**, lo que permite su evaluabilidad completa.

## Tablas de proyectos y grado de desarrollo

## BLOQUE TRANSVERSAL (TRA)

| **Proyecto** | **Clasificación** | **Valor añadido** |
| --- | --- | --- |
| TRA1 | Propuesta técnica con valor añadido | Sí (modo “simulación”) |
| TRA2 | Propuesta técnica con valor añadido | No |
| TRA3 | Propuesta técnica con valor añadido | No |
| TRA4 | Propuesta técnica incluida (desarrollo suficiente) | No |
| TRA5 | Propuesta técnica incluida (desarrollo suficiente) | Sí (mejora de seguridad y soporte) |
| TRA6 | Propuesta técnica con valor añadido | Sí (generación automática de una especificación OpenAPI) |
| TRA7 | Propuesta técnica incluida (desarrollo suficiente) | Sí (verificador automático de secretos expuestos) |
| TRA8 | Propuesta técnica incluida (desarrollo suficiente) | Sí (modo identificativo único para eventos de centro o de calendario escolar) |

## AULAS VIRTUALES (AV)

| **Proyecto** | **Clasificación** | **Valor añadido** |
| --- | --- | --- |
| AV1 | Propuesta técnica con valor añadido | Sí (“informe de salud post-upgrade” por isla) |
| AV2 | Propuesta técnica incluida (desarrollo suficiente) | Sí (perfiles para tráfico web y tareas internas) |
| AV3 | Propuesta técnica incluida (desarrollo suficiente) | Sí (ficha técnica por plugin con “decisión de ciclo de vida”) |
| AV4 | Propuesta técnica con valor añadido | Sí (cuadro de mando técnico de salud Moodle) |
| AV5 | Propuesta técnica incluida (desarrollo suficiente) | No |
| AV6 | Propuesta técnica incluida (desarrollo suficiente) | Sí (modo compacto) |
| AV7 | Propuesta técnica incluida (desarrollo suficiente) | No |
| AV8 | Propuesta técnica incluida (desarrollo suficiente) | Sí (calendario técnico de “campaña de inicio de curso”) |
| AV9 | Propuesta técnica incluida (desarrollo suficiente) | Sí (modo “dry-run”) |
| AV10 | Propuesta técnica incluida (desarrollo suficiente) | Sí (prueba automática de conectividad) |
| AV11 | Propuesta técnica con valor añadido | Sí (notificación por correo al docente) |
| AV12 | Propuesta técnica con valor añadido | Sí (opción de generar textos en distintos idiomas con IA) |
| AV13 | Propuesta técnica con valor añadido | No |
| AV14 | Propuesta técnica incluida (desarrollo suficiente) | Sí (mostrar NIA enmascarado cuando no sea imprescindible verlo completo) |
| AV15 | Propuesta técnica incluida (desarrollo suficiente) | Sí (SSO funcional desde el aplicativo móvil) |
| AV16 | Propuesta técnica incluida (desarrollo suficiente) | Sí (prueba de carga específica para Kuet) |
| AV17 | Propuesta técnica incluida (desarrollo suficiente) | Sí (modo de previsualización) |
| AV18 | Propuesta técnica incluida (desarrollo suficiente) | Sí (simulación previa “validar sin enviar”) |
| AV19 | Propuesta técnica incluida (desarrollo suficiente) | Sí (añadir estimación previa de impacto) |
| AV20 | Propuesta técnica incluida (desarrollo suficiente) | No |
| AV21 | Propuesta técnica incluida (desarrollo suficiente) | Sí (añadir scopes de API por tipo de operación) |

## MEDIATECA (MED)

| **Proyecto** | **Clasificación** | **Valor añadido** |
| --- | --- | --- |
| MED1 | Propuesta técnica con valor añadido | Sí (incorporar todos los subtítulos multiidioma en las búsquedas por Elastic) |
| MED2 | Propuesta técnica con valor añadido | Sí (“modo explicación”) |
| MED3 | Propuesta técnica incluida (desarrollo suficiente) | Sí (limitar la comprobación de sesión activa en ciertos casos) |
| MED4 | Propuesta técnica incluida (desarrollo suficiente) | Sí (guardado de versiones anteriores del proyecto) |
| MED5 | Propuesta técnica incluida (desarrollo suficiente) | Sí (comprobación previa del ZIP e informe al usuario) |
| MED6 | Propuesta técnica incluida (desarrollo suficiente) | Sí (modo “simulación”) |
| MED7 | Propuesta técnica incluida (desarrollo suficiente) | Sí (mostrar antes de enviar validación ligera en cliente para extensiones admitidas) |
| MED8 | Propuesta técnica incluida (desarrollo suficiente) | Sí (bloque “qué puedo hacer ahora”) |
| MED9 | Propuesta técnica incluida (desarrollo suficiente) | Sí (transcripción multiidioma) |
| MED10 | Propuesta técnica incluida (desarrollo suficiente) | Sí (indicador interno de confianza) |
| MED11 | Propuesta técnica incluida (desarrollo suficiente) | Sí (mapa vivo de deuda técnica) |

## CORREO WEB (COR)

| **Proyecto** | **Clasificación** | **Valor añadido** |
| --- | --- | --- |
| COR1 | Propuesta técnica con valor añadido | Sí (pantalla de diagnóstico) |
| COR2 | Propuesta técnica con valor añadido | Sí (botón “el resumen no es correcto”) |
| COR3 | Propuesta técnica incluida (desarrollo suficiente) | Sí (opción “mantener tono institucional”) |
| COR4 | Propuesta técnica incluida (desarrollo suficiente) | Sí (opción “no cambiar información factual”) |

## CLOUD (CLO)

| **Proyecto** | **Clasificación** | **Valor añadido** |
| --- | --- | --- |
| CLO1 | Propuesta técnica con valor añadido | No |
| CLO2 | Propuesta técnica con valor añadido | No |

## WORDPRESS (WPM)

| **Proyecto** | **Clasificación** | **Valor añadido** |
| --- | --- | --- |
| WPM1 | Propuesta técnica con valor añadido | Sí (panel técnico interno de versiones por componente) |
| WPM2 | Propuesta técnica incluida (desarrollo suficiente) | Sí (aviso de licencias a punto de caducar o pendientes de activar) |
| WPM3 | Propuesta técnica con valor añadido | Sí (procedimiento de correlación entre eventos del WAF y logs de WordPress) |
| WPM4 | Propuesta técnica incluida (desarrollo suficiente) | Sí (mejoras en captcha) |
| WPM5 | Propuesta técnica incluida (desarrollo suficiente) | Sí (política individualizada de 2FA) |

## VIDEO (VID)

| **Proyecto** | **Clasificación** | **Valor añadido** |
| --- | --- | --- |
| VID1 | Propuesta técnica con valor añadido | Sí (recolector de métricas por emisión) |
| VID2 | Propuesta técnica con valor añadido | Sí (regla de admisión progresiva para salas de invitados) |
| VID3 | Propuesta técnica incluida (desarrollo suficiente) | Sí (aviso de caducidad de grabaciones para responsables de centro) |
| VID4 | Propuesta técnica incluida (desarrollo suficiente) | Sí (plantilla de “evento de alta audiencia”) |

## SERVICIOS UNITARIOS

| **Proyecto** | **Clasificación** | **Valor añadido** |
| --- | --- | --- |
| EMP1 | Propuesta técnica con valor añadido | Sí (dashboard de salud de diferentes componentes) |
| LDAP | Propuesta técnica con valor añadido | Sí (panel de “contraseñas temporales activas”) |
| EXE | Propuesta técnica con valor añadido | Sí (plantilla “DUA ligero” y reproductor en la nube Nextcloud de EM) |
| FOR1 | Propuesta técnica incluida (desarrollo suficiente) | Sí (informe de salud de Formularios) |
| BUS1 | Propuesta técnica con valor añadido | Sí (modo de “búsquedas frecuentes sin resultados”) |
| ANI1 | Propuesta técnica incluida (desarrollo suficiente) | Sí (crear un “mapa de deuda técnica”) |

## RESTO DE SISTEMAS

| **Proyecto** | **Clasificación** | **Valor añadido** |
| --- | --- | --- |
| SYN | Propuesta técnica con valor añadido | Sí (evolución hacia API de Raíces) |
| CAU | Propuesta técnica incluida (desarrollo suficiente) | Sí (panel de verificación postactualización) |
| EDU | Propuesta técnica incluida (desarrollo suficiente) | Sí (Biblioteca interna ligera de seguridad para EducaSAAC,) |
| BAN | Propuesta técnica incluida (desarrollo suficiente) | Sí (panel de comprobación previa) |
| DTIC | Propuesta técnica incluida (desarrollo suficiente) | Sí (informe previo en HTML y CSV) |
| SEG | Propuesta técnica con valor añadido | Sí (panel de salud de edición) |
| ALB | Propuesta técnica incluida (desarrollo suficiente) | Sí (consola de revisión de respuestas sin datos personales) |
| AVI | Propuesta técnica incluida (desarrollo suficiente) | Sí (validación de seguridad previa) |
| FOR | Propuesta técnica incluida (desarrollo suficiente) | Sí (autencitación contra SSO) |
| BOL | Propuesta técnica incluida (desarrollo suficiente) | Sí (validación previa al envío) |
| AYU | Propuesta técnica incluida (desarrollo suficiente) | No |
| POR | Propuesta técnica con valor añadido | Sí (endpoint de validación previa) |
| WES | Propuesta técnica incluida (desarrollo suficiente) | Sí (matriz de páginas críticas) |
| IFZ | Propuesta técnica con valor añadido | Sí (catálogo vivo de componentes) |
| MAX | Propuesta técnica incluida (desarrollo suficiente) | No |
| GES | Propuesta técnica incluida (desarrollo suficiente) | Sí (validador previo de ficheros) |
| USO | Propuesta técnica con valor añadido | Sí (instalación de rybbit contra el SSO y script de generación de entorno para los centros solicitados) |
| MAM | Propuesta técnica incluida (desarrollo suficiente) | Sí (orquestador ligero de scripts accesible por VPN) |
| EML | Propuesta técnica con valor añadido | No |
| ABI | Propuesta técnica incluida (desarrollo suficiente) | No |

## WEKAN (WEK)

**La propuesta técnica de empresa_s para el grupo Wekan (WEK) se hace de forma agregada, incluyendo elementos de todos los subproyectos, pero sin diferenciarlos indivdualmente.** La propuesta de mejora es una propuesta global para el grupo ya que la mejora propuesta implica mejoras individuales para cada subproyecto.

<table>
  <tr>
    <th><strong>Proyecto</strong></th>
    <th><strong>Clasificación</strong></th>
    <th><strong>Valor añadido</strong></th>
  </tr>
  <tr>
    <td>WEK1</td>
    <td>Propuesta técnica incluida (desarrollo deficiente)</td>
    <td rowspan="4">Sí (docker compose con procedimiento de backup y rollback probado)</td>
  </tr>
  <tr>
    <td>WEK2</td>
    <td>Propuesta técnica incluida (desarrollo deficiente)</td>
  </tr>
  <tr>
    <td>WEK3</td>
    <td>Propuesta técnica incluida (desarrollo deficiente)</td>
  </tr>
  <tr>
    <td>WEK4</td>
    <td>Propuesta técnica incluida (desarrollo deficiente)</td>
  </tr>
</table>

## Conclusión del anexo

El análisis de la memoria técnica de empresa_s permite establecer, de forma objetiva, las siguientes conclusiones:

**1. Cobertura completa del Anexo II** 
La propuesta desarrolla la totalidad de los proyectos y subproyectos incluidos en el alcance del contrato, sin identificarse omisiones relevantes.

**2. Desarrollo técnico individualizado** 
A diferencia de enfoques genéricos o transversales, la propuesta presenta una **arquitectura y solución específica por sistema**, permitiendo evaluar cada proyecto de manera independiente y objetiva.

**3. Alto nivel de detalle y evaluabilidad** 
La inmensa mayoría de los proyectos incluyen:

- definición de arquitectura,

- mecanismos de integración,

- diseño de procesos,

- tratamiento de requisitos no funcionales,

- y propuestas de mejora

lo que garantiza su evaluabilidad directa sin necesidad de inferencias.

**4. Coherencia global del conjunto** 
Todos los proyectos se integran dentro de un modelo arquitectónico común, manteniendo:

- criterios homogéneos de seguridad,

- integración transversal,

- y consistencia en la gestión de usuarios y datos.

## Síntesis final

La propuesta de empresa_s presenta un **nivel de desarrollo técnico alto y homogéneo en prácticamente la totalidad de los proyectos**, con especial profundidad en los bloques críticos (TRA, AV, MED, CLO, VID).

En consecuencia:

- no se identifican proyectos sin desarrollar,

- no existen vacíos de evaluabilidad,

- y la cobertura del alcance es completa y verificable.

Este anexo refuerza de forma objetiva la valoración técnica elevada obtenida por la oferta, al evidenciar una **traducción sistemática del Anexo II en soluciones técnicas concretas, individualizadas y evaluables**.
