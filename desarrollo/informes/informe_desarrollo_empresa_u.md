# 1. INFORME TÉCNICO DE EVALUACIÓN

empresa_u

## 1.1. CAPÍTULO RESUMEN EJECUTIVO

### 1.1.1. Objeto del informe

El presente informe tiene por objeto la evaluación técnica de la propuesta presentada por la empresa_u en el procedimiento basado en el Sistema Dinámico de Adquisición SDA 26/2021, relativo a los servicios de desarrollo evolutivo, mantenimiento correctivo, soporte y mejora del ecosistema tecnológico Educamadrid.

La valoración se realiza exclusivamente conforme a los criterios establecidos en el Documento de Invitación, en particular los contemplados en su apartado 7.2, relativos a los criterios sometidos a juicio de valor, sin considerar en ningún caso aspectos económicos ni capacidades no acreditadas en la documentación aportada.

### 1.1.2. Metodología de evaluación

La evaluación se ha desarrollado mediante un enfoque estructurado en dos fases.

En primer lugar, se ha realizado un análisis técnico exhaustivo del contenido de la memoria presentada, examinando de forma individualizada el grado de desarrollo de las soluciones propuestas para los distintos proyectos definidos en el Anexo II del pliego.

En segundo lugar, dicho análisis se ha trasladado al sistema de valoración previsto en el Documento de Invitación, asignando puntuaciones en función del nivel real de adecuación observado y atendiendo de forma prioritaria al principio de evaluabilidad, entendido como la capacidad de las propuestas para ser verificadas a partir de elementos concretos y objetivos.

Este enfoque se ajusta a lo dispuesto en la normativa de contratación pública, en particular al artículo 145 y siguientes de la Ley 9/2017 de Contratos del Sector Público, que exige que la valoración se fundamente en criterios objetivos y verificables.

### 1.1.3. Síntesis técnica de la propuesta

Del análisis realizado se concluye que la propuesta presentada por la empresa_u presenta una apariencia formal extensa, pero carece del nivel de desarrollo técnico necesario para su evaluación objetiva.

En términos generales, la memoria se caracteriza por un predominio de contenido descriptivo frente a contenido técnico, por la ausencia de desarrollo en la mayoría de los proyectos incluidos en el Anexo II y por la presencia de fragmentos no finalizados que evidencian que el documento no ha sido completado ni validado de forma adecuada antes de su presentación.

En particular, se ha comprobado que únicamente se desarrollan parcialmente tres proyectos, mientras que el resto de los requisitos se mencionan sin desarrollo técnico o se sustituyen por anotaciones internas que remiten a una futura ampliación del contenido, sin que dicha ampliación exista en la documentación entregada.

Este hecho impide establecer una relación directa entre los requisitos del pliego y las soluciones propuestas, lo que determina una limitación sustancial en la capacidad de evaluación de la oferta.

### 1.1.4. Deficiencias estructurales identificadas

El análisis realizado ha permitido identificar un conjunto de deficiencias que afectan de forma estructural a la propuesta.

En primer lugar, la ausencia de desarrollo técnico en la mayor parte de los proyectos del Anexo II impide verificar el cumplimiento de los requisitos del contrato, lo que supone una carencia esencial en la valoración de la solución técnica.

En segundo lugar, la falta de elementos verificables, tales como arquitecturas definidas, tecnologías concretas, mecanismos de integración o procedimientos operativos, imposibilita la evaluación objetiva de la propuesta en múltiples subcriterios.

En tercer lugar, la presencia de anotaciones internas propias de procesos de generación automática, no eliminadas ni sustituidas por contenido definitivo, pone de manifiesto que el documento no ha sido objeto de una revisión técnica suficiente, generando una incertidumbre objetiva sobre la coherencia y fiabilidad de la propuesta.

Finalmente, en relación con la planificación, se constata la ausencia de un diagrama de Gantt o herramienta equivalente que permita verificar la secuencia temporal de ejecución de los trabajos, lo que impide acreditar la viabilidad del servicio desde el punto de vista organizativo y temporal.

### 1.1.5. Resultado de la valoración

Como resultado de la aplicación del baremo establecido en el Documento de Invitación, la propuesta obtiene las siguientes puntuaciones:

Solución técnica ofertada: 2,20 puntos sobre 15 
**Planificación del servicio: 1,50 puntos sobre 15**

**Resultado total: 3,70 puntos sobre un máximo de 30 puntos**

### 1.1.6. Conclusión del resumen ejecutivo

La propuesta técnica presentada por la empresa_u no acredita un nivel suficiente de desarrollo técnico ni de planificación que permita garantizar la correcta ejecución del contrato.

Las carencias detectadas no tienen carácter puntual, sino estructural, afectando a la evaluabilidad de la solución, a la cobertura del alcance del contrato y a la viabilidad de las actuaciones propuestas.

**En consecuencia, la oferta se sitúa en niveles claramente insuficientes dentro del baremo de valoración técnica y no alcanza el umbral mínimo exigido, lo que justifica su exclusión del procedimiento conforme a lo establecido en el Documento de Invitación y en la normativa de contratación pública aplicable.**

## 1.2. CAPÍTULO INTRODUCCIÓN Y MARCO DE REFERENCIA

### 1.2.1. Objeto del informe

El presente informe tiene por objeto la evaluación técnica de la propuesta presentada por la empresa_u en el marco del procedimiento basado en el Sistema Dinámico de Adquisición SDA 26/2021, relativo a los servicios de desarrollo evolutivo, mantenimiento correctivo, soporte y mejora del ecosistema tecnológico Educamadrid.

La valoración se efectúa exclusivamente conforme a los criterios establecidos en el Documento de Invitación, en particular los contemplados en su apartado 7.2, relativos a los criterios sujetos a juicio de valor, en aplicación de los principios de objetividad, transparencia e igualdad de trato entre licitadores.

En ningún caso se han incorporado elementos externos a la documentación presentada ni se han realizado inferencias sobre capacidades no acreditadas expresamente en la memoria técnica aportada por el licitador.

### 1.2.2. Alcance del contrato

El alcance del contrato viene determinado por el Anexo II del Documento de Invitación, el cual define un conjunto amplio y heterogéneo de proyectos que afectan a múltiples sistemas interdependientes dentro del ecosistema Educamadrid.

Este ecosistema presenta un elevado grado de complejidad técnica y operativa, derivado de la concurrencia de diversos factores, entre los que cabe destacar la coexistencia de múltiples plataformas tecnológicas, la dependencia funcional entre sistemas, la necesidad de garantizar la continuidad permanente del servicio y la exigencia de integración coherente entre componentes de distinta naturaleza.

Este contexto obliga a que las propuestas técnicas no se limiten a la identificación de los sistemas o a la reproducción de los requisitos del pliego, sino que desarrollen soluciones concretas, técnicamente viables y adaptadas al entorno real, de forma que pueda verificarse su ejecución en condiciones operativas.

### 1.2.3. Metodología de evaluación

La evaluación técnica se ha llevado a cabo mediante un enfoque estructurado en dos fases claramente diferenciadas.

**En una primera fase, se ha analizado el contenido técnico de la propuesta presentada, examinando de forma individualizada el grado de desarrollo de las soluciones planteadas para cada uno de los proyectos definidos en el Anexo II.** Este análisis ha permitido identificar el nivel de concreción técnica, la coherencia interna de la propuesta y su adecuación al entorno específico del contrato.

En una segunda fase, los resultados del análisis técnico se han trasladado al sistema de valoración previsto en el apartado 7.2 del Pliego, asignando puntuaciones en función del grado de adecuación observado en cada uno de los subcriterios.

Este enfoque garantiza la trazabilidad entre los requisitos del pliego, el contenido de la propuesta y la puntuación finalmente asignada, evitando cualquier tipo de arbitrariedad en el proceso de evaluación.

### 1.2.4. Principios de valoración aplicados

La valoración realizada se ha fundamentado en los principios que rigen la contratación pública, en particular los recogidos en la Ley 9/2017 de Contratos del Sector Público.

En primer lugar, se ha aplicado el principio de objetividad, de forma que la evaluación se basa exclusivamente en el contenido verificable de la propuesta, sin incorporar valoraciones subjetivas ni interpretaciones extensivas.

En segundo lugar, se ha garantizado la trazabilidad de la valoración, asegurando la correspondencia directa entre el análisis técnico realizado y las puntuaciones asignadas en cada subcriterio.

En tercer lugar, se ha aplicado el principio de proporcionalidad, ajustando la puntuación al nivel real de desarrollo técnico de la propuesta, de forma que las carencias identificadas tengan un reflejo adecuado en la valoración final.

Finalmente, se ha aplicado el principio de evaluabilidad como criterio esencial, considerando únicamente aquellos elementos que pueden ser verificados de manera objetiva, excluyendo de la valoración aquellas declaraciones que no se encuentren sustentadas por un desarrollo técnico suficiente.

### 1.2.5. Consideraciones preliminares sobre la propuesta

El análisis preliminar de la propuesta presentada por la empresa_u pone de manifiesto una falta de correspondencia entre la extensión formal del documento y su contenido técnico efectivo.

A pesar de presentar una memoria de considerable longitud, el contenido analizado revela que una parte significativa de la propuesta no contiene desarrollo técnico evaluable, limitándose en numerosos casos a descripciones generales, referencias genéricas a metodologías estándar o indicaciones de contenido que no ha sido finalmente incorporado.

Asimismo, se han identificado en la documentación presentada elementos que evidencian la existencia de contenido no finalizado, incluyendo anotaciones internas en las que se indica que determinados apartados se completarían en una versión posterior, sin que dicho contenido esté presente en la propuesta evaluada.

Este hecho no constituye una mera deficiencia formal, sino que tiene un impacto directo en la evaluabilidad de la propuesta, al impedir comprobar el cumplimiento efectivo de una parte sustancial de los requisitos del pliego.

Adicionalmente, se han detectado incoherencias técnicas y planteamientos no adaptados al entorno real de Educamadrid, lo que evidencia una comprensión parcial de las condiciones operativas del servicio.

### 1.2.6. Conclusión del capítulo

El presente capítulo ha definido el marco jurídico, técnico y metodológico en el que se ha desarrollado la evaluación de la propuesta.

A partir de este punto, el informe se centra en el análisis detallado de la solución técnica ofertada, con el objetivo de determinar su grado real de adecuación a los requisitos del Documento de Invitación y su capacidad de ejecución en el contexto del contrato.

La información recogida en este capítulo resulta fundamental para interpretar correctamente las conclusiones técnicas desarrolladas en los apartados siguientes y la valoración final asignada a la propuesta.

## 1.3. ANÁLISIS DETALLADO DE LA SOLUCIÓN TÉCNICA

### 1.3.1. Consideraciones generales del análisis técnico

#### 1.3.1.1. Enfoque global de evaluación

El análisis técnico de la propuesta presentada por la empresa_u se ha llevado a cabo atendiendo a la capacidad de la oferta para traducir los requisitos definidos en el Anexo II en soluciones técnicas concretas, coherentes y verificables.

Este criterio resulta determinante en el presente procedimiento, dado que el objeto del contrato implica la ejecución de actuaciones complejas sobre un ecosistema tecnológico de elevada criticidad, lo que exige que las propuestas presentadas incorporen un nivel suficiente de desarrollo técnico que permita verificar su aplicabilidad real.

En este sentido, el análisis pone de manifiesto que la propuesta presenta una limitación estructural esencial, consistente en la falta de correspondencia entre la identificación de los requisitos y la definición de soluciones técnicas concretas que permitan su ejecución.

Este desfase entre lo que se declara y lo que realmente se desarrolla condiciona de forma directa la evaluación, al impedir verificar el cumplimiento efectivo de una parte sustancial del alcance del contrato.

#### 1.3.1.2. Nivel de desarrollo técnico

Uno de los aspectos más relevantes del análisis es el nivel de desarrollo técnico de la propuesta.

**A pesar de la extensión formal de la memoria, el contenido técnico real resulta claramente insuficiente en relación con el volumen y la complejidad de los requisitos definidos en el pliego.** En particular, se observa que únicamente se desarrollan de forma parcial algunos proyectos concretos, mientras que el resto del contenido se limita a referencias genéricas o a indicaciones de desarrollo futuro que no se materializan en la propuesta.

La propia memoria incluye anotaciones expresas en las que se indica que el análisis detallado de los requisitos se completaría en una versión posterior, lo que confirma que el documento no incorpora el desarrollo técnico necesario para su evaluación.

Desde el punto de vista técnico y jurídico, este hecho implica que dichos contenidos no pueden ser objeto de valoración, al no formar parte efectiva de la oferta presentada.

#### 1.3.1.3. Evaluabilidad de la propuesta

El principio de evaluabilidad constituye uno de los elementos clave en la valoración de las ofertas, especialmente en procedimientos sujetos a juicio de valor.

En el presente caso, la propuesta presenta una grave limitación en este ámbito, en la medida en que una parte sustancial de su contenido no permite ser evaluada de manera objetiva. Esta circunstancia se debe a la ausencia de elementos técnicos concretos que permitan verificar:

- los mecanismos de implementación de las soluciones

- las tecnologías que se utilizarán

- las arquitecturas que soportarán los sistemas

- los procedimientos operativos asociados

Adicionalmente, la inclusión de contenido no finalizado y de referencias a apartados inexistentes impide establecer una relación clara entre los requisitos del pliego y las soluciones propuestas.

En consecuencia, no se trata de una propuesta que pueda considerarse parcialmente incompleta, sino de una propuesta en la que una parte significativa del alcance no es evaluable conforme a los criterios del Documento de Invitación.

#### 1.3.1.4. Indicios de contenido no validado

El análisis de la propuesta permite identificar indicios objetivos de que parte del contenido ha sido generado mediante herramientas automáticas y no ha sido revisado ni completado antes de su presentación.

Estos indicios se materializan en la inclusión de anotaciones internas que hacen referencia a desarrollos futuros o a contenidos no incorporados, lo que evidencia que el documento no ha sido finalizado en su totalidad.

Desde un punto de vista técnico, esta circunstancia afecta directamente a la fiabilidad de la propuesta, ya que impide garantizar:

- la coherencia interna del documento

- la consistencia de los planteamientos técnicos

- la validación por parte del equipo responsable

Desde un punto de vista jurídico, este aspecto refuerza la imposibilidad de considerar como acreditadas aquellas capacidades que no se encuentran desarrolladas de forma expresa en la oferta, de acuerdo con el principio de que la valoración debe basarse exclusivamente en el contenido presentado.

#### 1.3.1.5. Adecuación al entorno técnico

La propuesta incorpora planteamientos que no se encuentran adecuadamente adaptados al entorno real de Educamadrid.

Se identifican, entre otros aspectos, la utilización de metodologías estándar que no contemplan las particularidades del ecosistema, así como la propuesta de soluciones cuya implementación resulta inviable en los términos planteados, debido a la complejidad operativa del sistema.

En particular, se proponen actuaciones con tiempos de ejecución incompatibles con el funcionamiento real del entorno, lo que evidencia una falta de conocimiento técnico aplicado.

Asimismo, se plantean cambios arquitectónicos de carácter estructural que exceden el alcance del contrato, como la transformación completa hacia modelos de microservicios, lo que introduce un desalineamiento significativo con los requisitos del pliego.

#### 1.3.1.6. Cobertura del Anexo II

El análisis del grado de cobertura del Anexo II constituye uno de los elementos más determinantes de la valoración.

En este caso, se ha constatado que la propuesta no desarrolla de forma efectiva la mayoría de los proyectos incluidos en el alcance del contrato, limitándose en muchos casos a su simple mención.

La existencia de apartados no desarrollados, junto con las referencias explícitas a contenidos no incorporados, impide verificar el cumplimiento del pliego en una gran parte del alcance definido.

Este hecho tiene un impacto directo sobre la valoración, en la medida en que el grado de cumplimiento de los requisitos no puede ser acreditado en ausencia de desarrollo técnico.

### 1.3.2. Análisis por bloques funcionales del Anexo II

#### 1.3.2.1. Bloque de servicios transversales (TRA)

El bloque de servicios transversales es uno de los pocos ámbitos en los que la propuesta presenta un cierto nivel de desarrollo técnico.

No obstante, dicho desarrollo se caracteriza por un enfoque predominantemente conceptual, basado en descripciones generales de funcionalidad, sin incorporar un nivel suficiente de detalle técnico que permita verificar la implementación de las soluciones.

Aunque se describen algunos aspectos relativos a la gestión de identidades o a la integración entre sistemas, no se definen de forma concreta las arquitecturas, los mecanismos de sincronización ni las tecnologías específicas que soportarán estos procesos.

En consecuencia, el bloque presenta una evaluabilidad limitada, en la medida en que, aun existiendo contenido, no permite comprobar de forma completa la viabilidad de las actuaciones descritas.

#### 1.3.2.2. Bloque de Aulas Virtuales (AV)

El análisis del bloque de Aulas Virtuales pone de manifiesto una cobertura claramente insuficiente.

**Únicamente se desarrollan de forma parcial algunos subproyectos concretos, mientras que el resto del bloque no cuenta con desarrollo técnico evaluable.** Esta circunstancia se agrava por la inclusión de anotaciones que indican que el contenido correspondiente no ha sido incorporado en la memoria.

La ausencia de desarrollo afecta a la mayor parte de los subproyectos definidos en este bloque, lo que impide verificar aspectos esenciales relacionados con el rendimiento, la escalabilidad o la integración del sistema.

Desde el punto de vista de la evaluación, este bloque no puede considerarse adecuadamente cubierto, al no existir información suficiente para determinar el grado de cumplimiento de los requisitos.

#### 1.3.2.3. Bloque de Mediateca (MED)

El bloque de Mediateca presenta una situación similar a la descrita anteriormente.

**La propuesta incorpora referencias a algunos subproyectos, pero su desarrollo se limita a descripciones generales sin concreción técnica suficiente.** Asimismo, la mayor parte de los proyectos incluidos en este ámbito no se encuentran desarrollados en la memoria.

La ausencia de detalles técnicos en aspectos clave, como la gestión de contenidos multimedia, los mecanismos de indexación o la arquitectura de almacenamiento, impide evaluar la viabilidad de las soluciones planteadas.

En consecuencia, el bloque no alcanza el nivel de desarrollo requerido para su valoración positiva.

#### 1.3.2.4. Resto de bloques funcionales

En relación con el resto de bloques definidos en el Anexo II, incluyendo sistemas de correo, plataformas cloud, videoconferencia y sistemas auxiliares, la propuesta no presenta desarrollo técnico evaluable.

En estos casos, la memoria se limita a mencionar los proyectos sin describir las soluciones asociadas, o bien remite a desarrollos futuros que no forman parte del documento presentado.

Dado que estos bloques representan una parte sustancial del alcance del contrato, su ausencia implica una carencia estructural que afecta de manera decisiva a la valoración global de la propuesta.

### 1.3.3. Conclusión del capítulo

El análisis detallado de la solución técnica presentada por la empresa_u pone de manifiesto la existencia de deficiencias estructurales que impiden considerar que la propuesta cumple los requisitos mínimos de desarrollo técnico exigidos por el pliego.

En particular, se concluye que:

- la propuesta no desarrolla de forma efectiva la mayoría de los proyectos definidos en el Anexo II,

- no incluye elementos técnicos suficientes para permitir su evaluación objetiva,

- presenta contenido no finalizado que impide verificar la coherencia de la solución,

- y manifiesta una falta de adaptación al entorno real de ejecución del contrato.

Estas deficiencias afectan de manera directa a la evaluabilidad de la propuesta y, en consecuencia, determinan de forma decisiva la puntuación asignada en el apartado relativo a la solución técnica.

## 1.4. VALORACIÓN DE LA PROPUESTA

### 1.4.1. Fundamentación de la valoración

La valoración técnica de la propuesta presentada por la empresa_u se ha llevado a cabo en estricta aplicación de los criterios establecidos en el apartado 7.2 del Documento de Invitación, tomando como base el análisis técnico detallado desarrollado en el Capítulo 2 del presente informe.

**El sistema de valoración previsto en el pliego establece un modelo de carácter cualitativo, sujeto a juicio de valor, que permite asignar puntuaciones dentro de rangos continuos en función del grado de adecuación observado en cada uno de los subcriterios definidos.** En consecuencia, la puntuación final no responde a una aplicación automática o aritmética, sino a un juicio técnico fundado, sustentado en elementos objetivos y verificables.

**En este contexto, la valoración se ha basado exclusivamente en el contenido de la memoria técnica presentada, sin incorporar inferencias, interpretaciones extensivas ni suposiciones respecto a capacidades futuras no reflejadas en la documentación.** Este criterio se ajusta al principio de objetividad recogido en la normativa de contratación pública, conforme al cual la evaluación debe fundamentarse en elementos concretos que permitan su verificación.

Debe subrayarse que la puntuación asignada ha sido condicionada de forma determinante por la falta de desarrollo técnico de la propuesta, la imposibilidad de evaluar una parte sustancial del contenido y la ausencia de planificación verificable, elementos todos ellos acreditados de forma expresa en el análisis técnico previo.

### 1.4.2. Valoración de la solución técnica ofertada

### 1.4.3. Arquitectura de la solución

La propuesta incorpora referencias a modelos arquitectónicos genéricos basados en enfoques de servicios y microservicios. **Sin embargo, el análisis técnico pone de manifiesto que dichas referencias no se acompañan de una definición operativa que permita verificar su aplicación real.**

No se desarrolla una arquitectura específica para los distintos sistemas del Anexo II, ni se incluyen diagramas, modelos de despliegue o mecanismos de integración que permitan comprender cómo se articularían las soluciones en el entorno real de ejecución. **Asimismo, se plantean transformaciones estructurales que exceden el alcance del contrato, sin una justificación técnica adecuada.**

Estas carencias impiden considerar que la arquitectura se encuentre definida en los términos exigidos por el pliego, dado que no permite evaluar su adecuación ni su viabilidad.

En consecuencia, la arquitectura debe valorarse en niveles mínimos, al tratarse de un planteamiento de carácter conceptual sin desarrollo técnico suficiente.

**Puntuación asignada**: 0,30 sobre 2

#### 1.4.3.1. Comprensión de los requisitos

La propuesta declara explícitamente un conocimiento completo del entorno y de los requisitos del contrato. **No obstante, el análisis realizado evidencia que esta afirmación no se traduce en una comprensión técnica efectiva.**

Se identifican incoherencias significativas entre los planteamientos teóricos y la realidad operativa del entorno, así como la utilización de metodologías estándar que no contemplan las particularidades de Educamadrid. **Asimismo, la ausencia de desarrollo en la mayoría de los proyectos del Anexo II pone de manifiesto que los requisitos no han sido interpretados ni desarrollados de forma adecuada.**

**Desde el punto de vista técnico, la comprensión de los requisitos debe manifestarse mediante la capacidad de definir soluciones concretas y adaptadas al contexto.** En este caso, dicha capacidad no queda acreditada.

Por tanto, la comprensión de los requisitos se considera insuficiente.

**Puntuación asignada**: 0,30 sobre 2

#### 1.4.3.2. Viabilidad de la propuesta

La viabilidad de una solución técnica exige la definición de mecanismos concretos que permitan su ejecución en condiciones reales.

**En la propuesta analizada, la mayoría de los proyectos carecen de desarrollo técnico, lo que impide verificar aspectos esenciales como la selección de tecnologías, la arquitectura de los sistemas o los procedimientos de implementación.** Asimismo, se incluyen referencias a actuaciones cuya ejecución no puede ser comprobada por falta de información.

Esta ausencia de definición técnica impide acreditar la viabilidad de la propuesta tanto desde el punto de vista técnico como operativo.

En consecuencia, la viabilidad no puede ser considerada como demostrada.

**Puntuación asignada**: 0,20 sobre 1

#### 1.4.3.3. Metodología

La propuesta incorpora una metodología basada en modelos estándar de desarrollo ágil, presentada de forma estructurada y coherente desde un punto de vista conceptual.

**No obstante, dicha metodología no se adapta de forma específica al entorno del contrato ni se vincula con soluciones técnicas concretas.** La ausencia de desarrollo en los proyectos limita su aplicabilidad práctica, al no existir una conexión entre los procesos metodológicos descritos y la ejecución real de las actuaciones.

A pesar de ello, la calidad formal de la exposición metodológica permite reconocer un valor parcial en este subcriterio.

**Puntuación asignada**: 0,40 sobre 1

#### 1.4.3.4. Rendimiento

El tratamiento del rendimiento en la propuesta se realiza de forma genérica, sin incorporar métricas, escenarios de carga ni mecanismos de validación que permitan evaluar el comportamiento de los sistemas en condiciones reales.

Además, se hace referencia a proyectos y funcionalidades que no han sido desarrollados en la propuesta, lo que impide valorar su impacto sobre el rendimiento global del sistema.

En consecuencia, el rendimiento no puede ser evaluado en términos objetivos, al carecer de elementos verificables.

**Puntuación asignada**: 0,20 sobre 1

#### 1.4.3.5. Satisfacción de los requisitos del Anexo II

Este subcriterio constituye el elemento de mayor peso dentro de la valoración de la solución técnica, al estar directamente vinculado con el cumplimiento del objeto del contrato.

El análisis detallado del Anexo II pone de manifiesto que la propuesta no desarrolla la mayoría de los proyectos incluidos en el alcance, limitándose en múltiples casos a su mención o a la inclusión de anotaciones internas que indican que el contenido correspondiente no ha sido incorporado.

La propia memoria incluye indicaciones explícitas de que el desarrollo de los requisitos se completaría en una versión posterior, lo que implica que dichos contenidos no forman parte de la oferta presentada y, por tanto, no pueden ser objeto de valoración.

Esta circunstancia impide verificar el cumplimiento del pliego en una parte sustancial del alcance del contrato, lo que determina una valoración claramente insuficiente en este subcriterio.

**Puntuación asignada**: 0,80 sobre 8

#### 1.4.3.6. Resultado del bloque de solución técnica

La puntuación total correspondiente al bloque de solución técnica, como resultado de la suma de los subcriterios anteriores, asciende a **2,20 puntos sobre un máximo de 15**.

### 1.4.4. Valoración de la planificación del servicio

#### 1.4.4.1. Consideraciones generales

La planificación del servicio constituye un elemento esencial para garantizar la correcta ejecución del contrato, especialmente considerando la complejidad del ecosistema Educamadrid, caracterizado por la concurrencia de múltiples proyectos interdependientes, la criticidad operativa de los sistemas y la necesidad de asegurar la continuidad del servicio.

De acuerdo con lo establecido en el Pliego, la planificación debe permitir verificar:

- la secuenciación temporal de los trabajos

- la coordinación entre actuaciones

- la asignación de recursos

- la gestión de riesgos y contingencias

- los mecanismos de control y trazabilidad

Sin embargo, del análisis de la memoria técnica de la empresa_u se constata que la planificación aportada presenta un carácter predominantemente **conceptual y declarativo**, sin incorporar los elementos estructurales necesarios para su evaluación objetiva.

En particular, la ausencia de una planificación temporal verificable constituye una carencia crítica que condiciona de manera determinante la valoración global del bloque.

#### 1.4.4.2. Calendario de los trabajos a desarrollar (11 puntos)

### 1.4.5. Análisis técnico

La propuesta presenta la planificación a través de una descripción genérica de fases del servicio, sin incorporar una estructuración temporal detallada de los trabajos a desarrollar.

En este sentido, se constata la **ausencia total de instrumentos de planificación operativa**, tales como:

- diagramas de Gantt

- cronogramas detallados

- planificación por proyectos o subproyectos

- definición de hitos intermedios y finales

- identificación de dependencias entre actividades

Asimismo, no se aporta información relativa a:

- la duración estimada de las actuaciones

- la secuenciación de los proyectos incluidos en el Anexo II

- la posible ejecución en paralelo de distintos trabajos

- la priorización de actuaciones en función de su criticidad

Esta ausencia impide verificar aspectos esenciales para la ejecución del contrato, en particular:

- la viabilidad temporal del servicio

- la correcta coordinación entre sistemas interdependientes

- la adecuación del calendario al volumen real de trabajo

Desde un punto de vista técnico, la planificación aportada no permite realizar un análisis objetivo de la ejecución del servicio en condiciones reales, al no existir elementos que permitan validar la coherencia entre alcance, recursos y tiempo.

Resulta especialmente relevante que el contrato objeto del procedimiento implica la ejecución simultánea de un número elevado de proyectos interrelacionados, lo que hace imprescindible una planificación temporal estructurada que no se encuentra presente en la propuesta.

### 1.4.6. Valoración

El calendario de trabajos no puede considerarse definido conforme a los requisitos del pliego, al tratarse de una descripción general sin desarrollo técnico verificable.

Se trata de una **deficiencia crítica**, al impedir acreditar la viabilidad temporal del servicio.

### 1.4.7. Puntuación asignada: 0,50 sobre 11

#### 1.4.7.1. Análisis de riesgos del proyecto (1 punto)

### 1.4.8. Análisis técnico

La propuesta incluye un apartado específico de análisis de riesgos, estructurado de acuerdo con metodologías estándar.

En dicho apartado se identifican:

- tipologías generales de riesgos

- referencias a procesos de seguimiento y control

- planteamientos genéricos de mitigación

No obstante, el análisis presenta las siguientes limitaciones:

- no se vincula con proyectos concretos del Anexo II

- no se cuantifica la probabilidad ni el impacto de los riesgos

- no se establecen mecanismos específicos de mitigación

- no se integra con la planificación del servicio

Pese a estas limitaciones, el contenido permite identificar un conocimiento básico de los enfoques metodológicos aplicables a la gestión de riesgos, siendo por tanto parcialmente evaluable.

### 1.4.9. Valoración

Se reconoce un valor técnico limitado, circunscrito al plano conceptual.

### 1.4.10. Puntuación asignada: 0,30 sobre 1

#### 1.4.10.1. Plan de gestión de contingencias (1 punto)

### 1.4.11. Análisis técnico

La propuesta contempla referencias genéricas a la gestión de contingencias, alineadas con los principios metodológicos descritos en la gestión de riesgos.

Sin embargo, no se identifica un desarrollo específico que permita considerar la existencia de un plan de contingencias operativo, en la medida en que:

- no se definen escenarios concretos de contingencia

- no se establecen protocolos de actuación

- no se asignan responsabilidades operativas

- no se determinan tiempos de respuesta ni mecanismos de recuperación

Asimismo, no existe vinculación entre las contingencias y los sistemas concretos del entorno Educamadrid, lo que limita la aplicabilidad real del planteamiento.

En consecuencia, la propuesta no permite verificar la capacidad efectiva de respuesta ante incidencias o situaciones críticas durante la ejecución del servicio.

### 1.4.12. Valoración

El plan de contingencias no se encuentra definido en términos evaluables, si bien se identifican referencias conceptuales básicas.

### 1.4.13. Puntuación asignada: 0,20 sobre 1

#### 1.4.13.1. Plan de gestión de la calidad del servicio (1 punto)

### 1.4.14. Análisis técnico

La propuesta incorpora un apartado específico relativo a la gestión de la calidad, estructurado conforme a metodologías ampliamente reconocidas.

En particular, se incluyen:

- principios generales de calidad

- referencias a procesos de control

- planteamientos de mejora continua

No obstante, el desarrollo presenta las siguientes limitaciones:

- ausencia de indicadores concretos (KPIs)

- falta de definición de métricas cuantificables

- inexistencia de mecanismos de seguimiento vinculados a entregables

- no integración con los proyectos del Anexo II

A pesar de ello, el contenido muestra una estructura formal coherente y alineada con buenas prácticas, lo que permite su consideración como parcialmente evaluable.

### 1.4.15. Valoración

Se reconoce valor metodológico formal, aunque con limitada aplicabilidad práctica.

### 1.4.16. Puntuación asignada: 0,30 sobre 1

#### 1.4.16.1. Trazabilidad del servicio (1 punto)

### 1.4.17. Análisis técnico

La propuesta hace referencia a mecanismos de trazabilidad dentro del marco general de gestión del servicio, incluyendo:

- seguimiento de actuaciones

- control de cambios

- registro de actividad

Sin embargo, no se desarrolla un modelo de trazabilidad completo que permita:

- vincular requisitos del Anexo II con desarrollos concretos

- identificar entregables asociados a cada actuación

- realizar seguimiento detallado por proyecto

- integrar la trazabilidad con la planificación y la ejecución

La ausencia de estos elementos limita la capacidad de verificar de forma objetiva el grado de cumplimiento del contrato.

No obstante, la existencia de referencias estructuradas permite reconocer un nivel básico de planteamiento metodológico.

### 1.4.18. Valoración

Se reconoce valor parcial, de carácter conceptual.

### 1.4.19. Puntuación asignada: 0,20 sobre 1

#### 1.4.19.1. Resultado del bloque de planificación

| **Subcriterio** | **Puntuación** |
| --- | --- |
| Calendario de trabajos | 0,50 / 11 |
| Análisis de riesgos | 0,30 / 1 |
| Plan de contingencias | 0,20 / 1 |
| Plan de calidad | 0,30 / 1 |
| Trazabilidad | 0,20 / 1 |
| **Total** | **1,50 / 15** |

### 1.4.20. Resultado final de la valoración

Como resultado de la evaluación realizada, la propuesta presentada por la empresa_u obtiene las siguientes puntuaciones:

**Solución técnica: 2,20 puntos sobre 15** 
**Planificación del servicio: 1,50 puntos sobre 15**

**La puntuación total asciende a 3,70 puntos sobre un máximo de 30.**

### 1.4.21. Consideración final de la valoración

La puntuación obtenida refleja de manera fiel el nivel de desarrollo técnico real de la propuesta, así como su falta de adecuación a los requisitos establecidos en el pliego.

Debe señalarse que esta valoración no se basa en la ausencia de determinados elementos aislados, sino en la existencia de carencias estructurales que afectan a la mayor parte del contenido de la propuesta, impidiendo su evaluación objetiva.

En consecuencia, la puntuación asignada responde a un análisis técnico coherente, proporcional y plenamente alineado con los criterios del Documento de Invitación, constituyendo una valoración sólida y defendible desde el punto de vista jurídico.

## 1.5. CONCLUSIONES Y PROPUESTA DE EXCLUSIÓN

### 1.5.1. Conclusión global de la evaluación técnica

El análisis técnico desarrollado a lo largo del presente informe permite concluir de manera fundada que la propuesta presentada por la empresa_u no alcanza el nivel mínimo exigido para garantizar la correcta ejecución del contrato en los términos establecidos en el Documento de Invitación.

Esta conclusión no se basa en deficiencias puntuales o en aspectos secundarios de la propuesta, sino en la existencia de carencias de carácter estructural que afectan a los elementos esenciales de la valoración técnica, en particular a la definición de la solución técnica, a la cobertura efectiva del alcance del contrato y a la planificación del servicio.

**En efecto, el análisis realizado ha puesto de manifiesto que la propuesta no desarrolla de forma efectiva la mayoría de los proyectos incluidos en el Anexo II, lo que impide verificar el cumplimiento de los requisitos del pliego y limita de forma determinante su evaluabilidad.** Asimismo, la ausencia de una planificación detallada y verificable, unida a la falta de concreción en la definición de las soluciones técnicas, imposibilita acreditar la viabilidad de la propuesta en condiciones reales de ejecución.

En consecuencia, la propuesta no puede considerarse adecuada a los requisitos definidos en el pliego, ni desde el punto de vista técnico ni desde el punto de vista operativo.

### 1.5.2. Coherencia entre análisis técnico y resultado de la valoración

La puntuación final obtenida, de 3,70 puntos sobre un máximo de 30, es el resultado directo del análisis técnico realizado, existiendo una correspondencia clara y trazable entre las deficiencias detectadas y la valoración asignada en cada uno de los subcriterios definidos en el apartado 7.2 del Documento de Invitación.

En particular, debe destacarse que las puntuaciones más bajas se han asignado en aquellos subcriterios que presentan mayor impacto en la ejecución del contrato, tales como la satisfacción de los requisitos del Anexo II, la arquitectura de la solución y la planificación del servicio.

Esta distribución de la puntuación responde a la aplicación del principio de proporcionalidad, en la medida en que las carencias identificadas afectan precisamente a los elementos esenciales del contrato, lo que justifica una reducción significativa de la valoración en dichos apartados.

Al mismo tiempo, se han reconocido de forma proporcional aquellos elementos de la propuesta que presentan cierto grado de desarrollo, como los apartados relativos a metodología, gestión de riesgos o calidad, evitando así una valoración arbitraria o desproporcionada.

En consecuencia, la puntuación final refleja de manera fiel el nivel real de desarrollo técnico de la propuesta, sin incurrir en desviaciones ni en sobrevaloraciones no justificadas.

### 1.5.3. Impacto de la falta de evaluabilidad en la valoración

Uno de los factores más determinantes en la valoración de la propuesta es la falta de evaluabilidad de una parte sustancial de su contenido.

**Tal y como se ha expuesto en el análisis técnico, la ausencia de desarrollo en numerosos proyectos, junto con la inclusión de contenido no finalizado, impide verificar de manera objetiva el cumplimiento de los requisitos del pliego.** Esta circunstancia tiene una consecuencia directa en la valoración, dado que, conforme a los principios de la contratación pública, únicamente pueden ser objeto de valoración aquellos elementos que se encuentren definidos de forma clara en la oferta.

Este criterio ha sido reiteradamente reconocido en la doctrina de los órganos de control y tribunales administrativos de contratación, que establecen que no puede atribuirse valoración positiva a aquellas partes de la oferta que no se encuentren suficientemente desarrolladas o que requieran de interpretaciones o suposiciones por parte del órgano evaluador.

En este contexto, la imposibilidad de evaluar una parte significativa de la propuesta no constituye una apreciación subjetiva, sino una consecuencia directa del contenido presentado, lo que justifica plenamente la puntuación reducida asignada.

### 1.5.4. Relevancia de las deficiencias detectadas en la planificación

La planificación del servicio constituye un elemento esencial para garantizar la correcta ejecución del contrato, especialmente en un entorno de elevada complejidad como el de Educamadrid.

La ausencia de un diagrama de Gantt o de cualquier herramienta equivalente que permita representar de manera estructurada la secuencia temporal de las actuaciones impide verificar aspectos fundamentales como la distribución de los trabajos, la identificación de dependencias entre tareas o la viabilidad de los plazos propuestos.

Esta carencia no puede considerarse menor o subsanable mediante interpretaciones, dado que el propio pliego establece la necesidad de aportar una planificación clara y verificable como criterio de valoración.

En consecuencia, la falta de planificación estructurada debe ser considerada como una deficiencia crítica que afecta de forma directa a la puntuación del bloque de planificación del servicio y, por extensión, al resultado global de la evaluación.

### 1.5.5. Justificación de la insuficiencia técnica de la propuesta

Atendiendo a lo expuesto en los apartados anteriores, puede afirmarse que la propuesta presentada por la empresa_u no alcanza un nivel técnico suficiente para garantizar la correcta ejecución del contrato.

Esta insuficiencia se manifiesta de forma clara en los siguientes aspectos:

- la ausencia de desarrollo técnico en la mayor parte de los proyectos del Anexo II,

- la falta de correspondencia entre requisitos y soluciones,

- la imposibilidad de verificar la viabilidad técnica de las actuaciones,

- la inexistencia de una planificación detallada y verificable,

- y la presencia de contenido no finalizado que impide garantizar la coherencia de la propuesta.

Estos elementos no solo limitan la evaluación de la propuesta, sino que constituyen indicios objetivos de que la misma no reúne las condiciones necesarias para asegurar el cumplimiento de las obligaciones derivadas del contrato.

### 1.5.6. Aplicación del umbral mínimo de calidad

El Documento de Invitación establece la existencia de un umbral mínimo de calidad en los criterios sujetos a juicio de valor, cuya superación resulta imprescindible para continuar en el procedimiento de adjudicación.

Este umbral se configura como un requisito objetivo, orientado a garantizar que únicamente las ofertas que acrediten un nivel técnico suficiente puedan ser consideradas en fases posteriores del procedimiento.

Conforme a lo dispuesto en el artículo 146.3 de la Ley 9/2017 de Contratos del Sector Público, la fijación de umbrales mínimos en los criterios cualitativos es plenamente válida y su aplicación debe realizarse de forma automática una vez constatado su incumplimiento.

En el presente caso, la propuesta analizada obtiene una puntuación total de 3,70 puntos sobre 30, situándose de forma muy significativa por debajo del umbral mínimo exigido.

En consecuencia, no existe margen de apreciación o discrecionalidad en este punto, debiendo aplicarse de forma directa la consecuencia prevista en el pliego para este supuesto.

### 1.5.7. Propuesta de exclusión

A la vista de lo expuesto, procede proponer la exclusión de la oferta presentada por la empresa_u del procedimiento de adjudicación, al no alcanzar el umbral mínimo de calidad exigido en los criterios sujetos a juicio de valor.

Esta propuesta de exclusión se fundamenta en:

- la insuficiencia técnica de la propuesta,

- la falta de evaluabilidad de una parte sustancial de su contenido,

- la ausencia de planificación verificable,

- y la puntuación obtenida, inferior al umbral mínimo establecido en el pliego.

Debe subrayarse que esta exclusión no tiene carácter discrecional, sino que constituye la consecuencia directa y necesaria de la aplicación de los criterios establecidos en el pliego y en la normativa vigente.

### 1.5.8. Conclusión final

El presente informe pone de manifiesto, mediante un análisis técnico detallado y una valoración fundamentada, que la propuesta presentada por la empresa_u no acredita un nivel suficiente de calidad técnica para garantizar la correcta ejecución del contrato.

La puntuación obtenida refleja de manera fiel esta insuficiencia, situándose claramente por debajo del umbral mínimo exigido, lo que determina su exclusión del procedimiento.

El informe se presenta con la debida motivación técnica, trazabilidad en la valoración y coherencia con los criterios del pliego, constituyendo una base sólida, objetiva y defendible frente a cualquier eventual alegación en el ámbito del procedimiento de contratación.

## 1.6. CAPÍTULO PROPUESTA FINAL Y FUNDAMENTACIÓN DE LA DECISIÓN

### 1.6.1. Objeto del capítulo

El presente capítulo tiene por objeto formular la propuesta final derivada del análisis técnico realizado, integrando las conclusiones obtenidas en los capítulos anteriores y estableciendo de forma expresa las consecuencias administrativas que se derivan de la valoración efectuada.

Asimismo, se incluye la fundamentación técnica y jurídica de la decisión adoptada, con el fin de garantizar su plena coherencia con el Documento de Invitación y su adecuación a los principios que rigen la contratación pública.

### 1.6.2. Síntesis de la valoración técnica

Como resultado del proceso de evaluación desarrollado, la propuesta presentada por la empresa_u ha obtenido una puntuación total de 3,70 puntos sobre un máximo de 30, correspondiente a los criterios sujetos a juicio de valor definidos en el apartado 7.2 del Pliego.

Esta puntuación se deriva de la aplicación conjunta de los distintos subcriterios de valoración y refleja de manera fiel el nivel de desarrollo técnico de la propuesta, conforme a los siguientes aspectos fundamentales:

la ausencia de desarrollo técnico en la mayor parte de los proyectos del Anexo II, 
la imposibilidad de evaluar objetivamente una parte sustancial del contenido, 
la falta de definición de soluciones técnicas aplicables al entorno real del contrato, 
la inexistencia de una planificación verificable que permita acreditar la viabilidad temporal del servicio, 
y la presencia de contenido no finalizado que impide garantizar la coherencia y fiabilidad de la propuesta.

En su conjunto, estos elementos configuran una oferta que no alcanza el nivel mínimo exigido en términos de calidad técnica.

### 1.6.3. Aplicación del criterio de umbral mínimo

El Documento de Invitación establece la obligación de que las ofertas sometidas a juicio de valor alcancen un umbral mínimo de calidad técnica como requisito necesario para su continuidad en el procedimiento.

Este umbral, fijado en el 50 por ciento de la puntuación máxima asignable a dichos criterios, constituye un elemento objetivo de evaluación orientado a garantizar que únicamente las propuestas con un nivel técnico suficiente puedan ser consideradas en fases posteriores del procedimiento de adjudicación.

De acuerdo con lo establecido en la normativa de contratación pública, en particular el artículo 146.3 de la Ley 9/2017 de Contratos del Sector Público, la fijación y aplicación de umbrales mínimos en los criterios cualitativos es plenamente válida, debiendo aplicarse de forma automática una vez constatado su incumplimiento.

En el presente caso, la propuesta evaluada se sitúa muy por debajo del umbral mínimo fijado, sin que exista margen de interpretación que permita considerar su superación.

### 1.6.4. Fundamentación jurídica de la decisión

La decisión propuesta se fundamenta en los principios recogidos en la legislación de contratación pública, entre los que destacan el principio de objetividad, el principio de igualdad de trato y el principio de transparencia.

En primer lugar, la objetividad de la valoración queda garantizada por el hecho de que la puntuación asignada se basa exclusivamente en el contenido de la oferta presentada, sin incorporar valoraciones externas ni consideraciones subjetivas.

En segundo lugar, la igualdad de trato se ha asegurado mediante la aplicación homogénea de los mismos criterios de evaluación a todas las ofertas analizadas, sin introducir variaciones en la metodología ni en los parámetros de valoración.

En tercer lugar, la transparencia se materializa en la trazabilidad completa entre el análisis técnico realizado y la puntuación asignada, permitiendo reconstruir de manera clara y precisa el razonamiento que sustenta la decisión adoptada.

Asimismo, debe destacarse que la exclusión propuesta no constituye una decisión discrecional del órgano de contratación, sino una consecuencia directa de la aplicación de los criterios establecidos en el pliego, una vez verificado el incumplimiento del umbral mínimo de calidad técnica.

### 1.6.5. Propuesta final

A la vista de lo expuesto en el presente informe, se formula la siguiente propuesta:

proceder a la exclusión de la oferta presentada por la empresa_u del procedimiento de adjudicación, al no alcanzar el umbral mínimo de puntuación establecido para los criterios sujetos a juicio de valor.

Esta propuesta se fundamenta en la insuficiencia técnica acreditada de la oferta, en la falta de evaluabilidad de una parte sustancial de su contenido y en la imposibilidad de verificar la viabilidad de las soluciones planteadas, circunstancias todas ellas debidamente acreditadas a lo largo del presente informe.

### 1.6.6. Consideraciones finales

El presente informe ha sido elaborado conforme a criterios técnicos objetivos, garantizando la coherencia entre el análisis realizado y la valoración asignada, así como la adecuación de la decisión propuesta a los requisitos establecidos en el pliego y en la normativa aplicable.

Las conclusiones alcanzadas se encuentran debidamente motivadas y fundamentadas, constituyendo una base sólida y defendible en el marco del procedimiento de contratación, tanto desde el punto de vista técnico como jurídico.

En consecuencia, la propuesta de exclusión formulada responde a la aplicación estricta de los criterios de valoración y no a una apreciación discrecional, lo que garantiza su solidez frente a cualquier eventual alegación en vía administrativa o jurisdiccional.

## 1.7. ANEXO RELACIÓN DE PROYECTOS Y GRADO DE DESARROLLO EN LA PROPUESTA TÉCNICA

El presente anexo tiene por objeto identificar, de forma sistemática, los proyectos y subproyectos incluidos en la memoria técnica presentada por la empresa_u, indicando para cada uno de ellos el grado real de desarrollo observado en la documentación aportada.

## 1.8. Criterio de clasificación

Se utiliza la siguiente tipología:

- **Propuesta técnica desarrollada de forma general** → incluye solución detallada, arquitectura, tareas y/o casos de uso

- **Mención sin desarrollo completo** → aparece citado o contextualizado, pero sin desarrollo técnico suficiente

- **Pendiente de desarrollo en el documento** → el propio documento indica que se desarrollará posteriormente en una versión más completa de la Memoria Técnica.

- **No mencionado** → no aparece de forma identificable

## 1.9. BLOQUE TRANSVERSAL (TRA)

| **Proyecto** | **Clasificación** |
| --- | --- |
| TRA1 | Propuesta técnica desarrollada de forma general |
| TRA2 | Propuesta técnica desarrollada de forma general |
| TRA3 | Propuesta técnica desarrollada de forma general |
| TRA4 | Propuesta técnica desarrollada de forma general |
| TRA5 | Propuesta técnica desarrollada de forma general |
| TRA6 | Propuesta técnica desarrollada de forma general |
| TRA7 | Propuesta técnica desarrollada de forma general |
| TRA8 | Propuesta técnica desarrollada de forma general |

## 1.10. AULAS VIRTUALES (AV)

| **Proyecto** | **Clasificación** |
| --- | --- |
| AV1 | Propuesta técnica desarrollada de forma general |
| AV2 | Propuesta técnica desarrollada de forma general |
| AV3 | Pendiente de desarrollo en el documento |
| AV4 | Pendiente de desarrollo en el documento |
| AV5 | Pendiente de desarrollo en el documento |
| AV6 | Pendiente de desarrollo en el documento |
| AV7 | Pendiente de desarrollo en el documento |
| AV8 | Pendiente de desarrollo en el documento |
| AV9 | Pendiente de desarrollo en el documento |
| AV10 | Propuesta técnica desarrollada de forma general |
| AV11 | Propuesta técnica desarrollada de forma general |
| AV12 | Mención sin desarrollo completo |
| AV13 | Propuesta técnica desarrollada de forma general |
| AV14 | Pendiente de desarrollo en el documento |
| AV15 | Pendiente de desarrollo en el documento |
| AV16 | Pendiente de desarrollo en el documento |
| AV17 | Pendiente de desarrollo en el documento |
| AV18 | Pendiente de desarrollo en el documento |
| AV19 | Pendiente de desarrollo en el documento |
| AV20 | Pendiente de desarrollo en el documento |
| AV21 | Pendiente de desarrollo en el documento |

## 1.11. MEDIATECA (MED)

| **Proyecto** | **Clasificación** |
| --- | --- |
| MED1 | Propuesta técnica desarrollada de forma general |
| MED2 | Propuesta técnica desarrollada de forma general |
| MED3 | Pendiente de desarrollo en el documento |
| MED4 | Pendiente de desarrollo en el documento |
| MED5 | Pendiente de desarrollo en el documento |
| MED6 | Pendiente de desarrollo en el documento |
| MED7 | Pendiente de desarrollo en el documento |
| MED8 | Pendiente de desarrollo en el documento |
| MED9 | Pendiente de desarrollo en el documento |
| MED10 | Propuesta técnica desarrollada de forma general |
| MED11 | Mención sin desarrollo completo |

## 1.12. CORREO WEB (COR)

| **Proyecto** | **Clasificación** |
| --- | --- |
| COR1 | Mención sin desarrollo completo (aparece en solución técnica y planificación) |
| COR2 | Mención sin desarrollo completo (IA en correo) |
| COR3 | Mención sin desarrollo completo |
| COR4 | Mención sin desarrollo completo |

## 1.13. CLOUD (CLO)

| **Proyecto** | **Clasificación** |
| --- | --- |
| CLO1 | Mención sin desarrollo completo |
| CLO2 | Mención sin desarrollo completo |

## 1.14. WORDPRESS MULTISITE (WPM)

| **Proyecto** | **Clasificación** |
| --- | --- |
| WPM1 | Mención sin desarrollo completo |
| WPM2 | Mención sin desarrollo completo |
| WPM3 | Propuesta técnica desarrollada de forma general (WAF y seguridad) |
| WPM4 | Mención sin desarrollo completo |
| WPM5 | Propuesta técnica desarrollada de forma general (caso de uso operativo) |

## 1.15. VIDEO (VID)

| **Proyecto** | **Clasificación** |
| --- | --- |
| VID1 | Mención sin desarrollo completo |
| VID2 | Mención sin desarrollo completo |
| VID3 | Mención sin desarrollo completo |
| VID4 | Mención sin desarrollo completo |

## 1.16. SERVICIOS UNITARIOS

| **Proyecto** | **Clasificación** |
| --- | --- |
| EMP1 | Mención sin desarrollo completo |
| LDAP | Propuesta técnica desarrollada de forma general (seguridad e integración) |
| EXE | Mención sin desarrollo completo |
| FOR1 | Mención sin desarrollo completo |
| BUS1 | Propuesta técnica desarrollada de forma general (optimización búsquedas) |
| ANI1 | Mención sin desarrollo completo |

## 1.17. RESTO DE SERVICIOS TRANSVERSALES

| **Proyecto** | **Clasificación** |
| --- | --- |
| SYN | Mención sin desarrollo completo |
| CAU | Mención sin desarrollo completo |
| EDU | Mención sin desarrollo completo |
| BAN | Mención sin desarrollo completo |
| DTIC | Mención sin desarrollo completo |
| SEG | Mención sin desarrollo completo |
| ALB | Propuesta técnica desarrollada de forma general (caso IA detallado) |
| AVI | Mención sin desarrollo completo |
| FOR | Mención sin desarrollo completo |
| BOL | Mención sin desarrollo completo |
| AYU | Mención sin desarrollo completo |
| POR | Mención sin desarrollo completo |
| WES | Mención sin desarrollo completo |
| IFZ | Propuesta técnica desarrollada de forma general (framework UI) |
| MAX | Mención sin desarrollo completo |
| GES | Mención sin desarrollo completo |
| USO | Mención sin desarrollo completo |
| MAM | Mención sin desarrollo completo |
| EML | Mención sin desarrollo completo |
| ABI | Mención sin desarrollo completo |

## 1.18. WEKAN (WEK)

| **Proyecto** | **Clasificación** |
| --- | --- |
| WEK1 | Pendiente de desarrollo en el documento |
| WEK2 | Pendiente de desarrollo en el documento |
| WEK3 | Pendiente de desarrollo en el documento |
| WEK4 | Pendiente de desarrollo en el documento |

## 1.19. Conclusión del anexo

La tabla anterior pone de manifiesto que únicamente un número muy reducido de proyectos incluye algún grado de desarrollo técnico, y en la mayoría de los casos dicho desarrollo es parcial y no alcanza el nivel requerido para su evaluación completa.

La gran mayoría de los subproyectos del Anexo II se encuentran en una de las siguientes situaciones:

- mencionados sin desarrollo técnico suficiente, o

- directamente no incluidos en la memoria, siendo sustituidos por indicaciones de desarrollo futuro.

Esta circunstancia confirma de manera objetiva y documentada la falta de cobertura efectiva del alcance del contrato por parte de la propuesta presentada, así como la imposibilidad de evaluar de forma completa su adecuación a los requisitos del pliego, reforzando las conclusiones técnicas recogidas en el informe principal.
